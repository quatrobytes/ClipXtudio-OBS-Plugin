...
void serveOnce(QTcpServer &server, QByteArray body, int status,
	       QByteArray contentType)
{
	QObject::connect(
		&server, &QTcpServer::newConnection, &server,
		[&server, body = std::move(body), status,
		 contentType = std::move(contentType)] {
			auto *socket = server.nextPendingConnection();
			QObject::connect(
				socket, &QTcpSocket::readyRead, socket,
				[socket, body, status, contentType] {
					socket->readAll();
					const auto response =
						QByteArray("HTTP/1.1 ") +
...
int main(int argc, char **argv)
{
	...
	clipcoach::test::expect(
		unavailableManifest.listen(QHostAddress::LocalHost) &&
	...
	const auto installerHash =
		QString::fromLatin1(QCryptographicHash::hash(
			installerBytes, QCryptographicHash::Sha256).toHex());
	const auto manifest =
		QStringLiteral(
			R"({"version":"0.5.17","download_url":"%1","release_notes_url":"","sha256":"%2","size_bytes":%3})")
			.arg(urlFor(installerServer, QStringLiteral("/setup.exe")).toString(),
			     installerHash)
			.arg(installerBytes.size())
			.toUtf8();
	serveOnce(unavailableManifest, QByteArrayLiteral("{}"), 404,
		  QByteArrayLiteral("application/json"));
	serveOnce(validManifest, manifest, 200,
		  QByteArrayLiteral("application/json"));
	serveOnce(installerServer, installerBytes, 200,
		  QByteArrayLiteral("application/octet-stream"));
	serveOnce(noPublishedRelease,
		  QByteArrayLiteral(
	...
	emptyCatalogChecker.check(
		QStringLiteral("0.5.16"),
	...
	application.exec();
	clipcoach::test::expect(
		emptyCheckCompleted && emptyUpdate.success &&
			!emptyUpdate.updateAvailable && emptyUpdate.errorCode.isEmpty(),
	...
	clipcoach::test::expect(checker.configured(),
			      "loopback update endpoints must be allowed for local QA");

	bool checkCompleted = false;
	clipcoach::network::UpdateCheckResult update;
	checker.check(QStringLiteral("0.5.16"),
		      [&application, &checkCompleted, &update](auto result) {
	...
	application.exec();
	clipcoach::test::expect(
		checkCompleted && update.success && update.updateAvailable &&
			update.latestVersion == QStringLiteral("0.5.17") &&
			update.sha256 == installerHash &&
			update.sizeBytes == installerBytes.size(),
	...
	downloader.download(
		update, destination, {},
		[&application, &downloadCompleted](auto result) {
	...
	clipcoach::test::expect(
		downloadCompleted && downloaded.open(QIODevice::ReadOnly) &&
