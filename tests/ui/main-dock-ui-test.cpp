...
int main(int argc, char **argv)
{
	...
	const auto *headerStatus =
		dock.findChild<QLabel *>(QStringLiteral("headerStatusBadge"));
	const auto *headerRemoteAuthenticate = dock.findChild<QPushButton *>(
		QStringLiteral("headerRemoteAuthenticateButton"));
	const auto *headerRemoteOpen = dock.findChild<QPushButton *>(
		QStringLiteral("headerRemoteOpenButton"));
	const auto *demoGuide =
		dock.findChild<QPushButton *>(QStringLiteral("headerDemoGuideButton"));
	const auto *headerPro =
		dock.findChild<clipcoach::ui::ProBadge *>(QStringLiteral("headerProBadge"));
	const auto *planUpgrade =
		dock.findChild<QPushButton *>(QStringLiteral("planUpgradeButton"));
	...
	const auto *footerUpdate =
		dock.findChild<QPushButton *>(QStringLiteral("footerCheckForUpdatesButton"));
	const auto *updateIndicator =
		dock.findChild<QLabel *>(QStringLiteral("updateAvailableIndicator"));
	const auto *footerReplay =
		dock.findChild<QPushButton *>(QStringLiteral("footerReplayBufferButton"));
	const auto *footerClip = dock.findChild<QPushButton *>(QStringLiteral("footerSaveClipButton"));
	const auto *captureRemote =
		dock.findChild<QPushButton *>(QStringLiteral("captureRemoteClipperButton"));
	const auto *dockHeader = dock.findChild<QWidget *>(QStringLiteral("DockHeader"));
	const auto *statusContainer =
		dock.findChild<QWidget *>(QStringLiteral("DockStatusContainer"));
	auto *previewOnly =
		dock.findChild<QPushButton *>(QStringLiteral("verticalPreviewOnlyButton"));
	auto *remoteAuthOverlay =
		dock.findChild<QWidget *>(QStringLiteral("remoteAuthOverlay"));
	auto *remoteAuthProgress =
		dock.findChild<QProgressBar *>(QStringLiteral("remoteAuthProgress"));
	...
	clipcoach::test::expect(
		remoteAuthOverlay != nullptr && remoteAuthOverlay->isHidden() &&
	...
	clipcoach::test::expect(
		headerRemoteAuthenticate != nullptr && headerRemoteOpen != nullptr &&
			headerRemoteAuthenticate->text() == QString::fromUtf8(
				clipcoach::ui::strings::kHeaderRemoteAuthenticate) &&
			headerRemoteOpen->text() == QString::fromUtf8(
				clipcoach::ui::strings::kHeaderRemoteOpen),
		"the header must provide short authenticate and Remote Clipper buttons");
	clipcoach::test::expect(
		headerStatus != nullptr && headerStatus->text() == QString::fromUtf8(
			clipcoach::ui::strings::kHeaderBufferInactive),
		"the header must show a concise Replay Buffer state");
	clipcoach::test::expect(tabBar->currentIndex() == 0,
			       "Vertical must be the default primary tab");
	...
	clipcoach::test::expect(
		dock.findChild<QLabel *>(QStringLiteral("mainTabBadge")) == nullptr,
	...
	clipcoach::test::expect(
		captureRemote != nullptr &&
			captureRemote->text() == QString::fromUtf8(
				clipcoach::ui::strings::kSettingsRemoteOpenAccount),
		"capture tab must expose the localized Remote Clipper account action");
	clipcoach::test::expect(
		dock.findChild<QPushButton *>(
			QStringLiteral("footerRemoteClipperButton")) == nullptr,
	...
	auto *remotePulseTimer =
		dock.findChild<QTimer *>(QStringLiteral("remoteStatusPulseTimer"));
	clipcoach::test::expect(
		headerRemoteAuthenticate->property("authenticated").toBool() &&
			headerRemoteAuthenticate->text() == QString::fromUtf8(
				clipcoach::ui::strings::kHeaderRemoteAuthenticated) &&
			headerRemoteOpen->property("remoteState").toString() ==
				QStringLiteral("online") &&
			!headerRemoteAuthenticate->icon().isNull() &&
			!headerRemoteOpen->icon().isNull() &&
	...
	connectingRemote.connection =
		clipcoach::remote::RemoteConnectionState::Connecting;
	dock.setRemoteClipperStatus(connectingRemote);
	clipcoach::test::expect(
		headerRemoteOpen->property("remoteState").toString() ==
			QStringLiteral("online") && remotePulseTimer->isActive(),
	...
	clipcoach::test::expect(
		markMoment->text() == QString::fromUtf8(
			clipcoach::ui::strings::kCaptureSaveHorizontal),
	...
	clipcoach::test::expect(
		footer != nullptr && footerVersion == nullptr &&
			footerCredits != nullptr && footerUpdate != nullptr &&
			updateIndicator != nullptr && updateIndicator->isHidden() &&
			footerReplay != nullptr && footerClip != nullptr,
	...
	clipcoach::test::expect(footerLayout != nullptr,
				"fixed footer must use one non-wrapping horizontal row");
	clipcoach::test::expect(
		footerLayout->indexOf(footerReplay) <
				footerLayout->indexOf(footerCredits) &&
			footerLayout->indexOf(footerClip) <
				footerLayout->indexOf(footerCredits) &&
			footerLayout->indexOf(footerUpdate) <
				footerLayout->indexOf(footerCredits),
		"footer actions must stay left of developer credit");
	clipcoach::test::expect(
			footerReplay->height() == footerClip->height() &&
			footerClip->height() == footerUpdate->height() &&
			footerClip->height() ==
				clipcoach::ui::tokens::kFooterControlHeight,
		"all visible footer actions must use the same compact footer height");
	clipcoach::test::expect(
		footerReplay->geometry().center().y() == footerClip->geometry().center().y() &&
	...
	clipcoach::test::expect(
		footerReplay->text() == QStringLiteral("Replay") &&
	...
	previewOnly->click();
	application.processEvents();
	clipcoach::test::expect(previewOnly->isChecked(),
				"preview-only action must enter its checked state");
	clipcoach::test::expect(dockHeader->isHidden(),
				"preview-only mode must hide the dock header");
	clipcoach::test::expect(statusContainer->isHidden(),
				"preview-only mode must hide the status cards");
	clipcoach::test::expect(tabBar->isHidden(),
				"preview-only mode must hide the primary tab bar");
	clipcoach::test::expect(footer->isVisible(),
				"preview-only mode must keep the compact footer visible");
	clipcoach::test::expect(verticalPreview->isVisible(),
				"preview-only mode must keep the vertical camera visible");
	clipcoach::test::expect(previewOnly->isVisible(),
				"preview-only mode must keep a visible restore action");
	previewOnly->click();
	application.processEvents();
	clipcoach::test::expect(
		dockHeader->isVisible() && statusContainer->isVisible() &&
			tabBar->isVisible(),
	...
	clipcoach::test::expect(
		brandLabel != nullptr &&
			brandLabel->text().contains(QStringLiteral(" - v")) &&
	...
	clipcoach::test::expect(
		planUpgrade != nullptr && !planUpgrade->isHidden() &&
	...
	clipcoach::test::expect(
		planUpgrade->property("externalUrl").toString().endsWith(QStringLiteral("/#pricing")),
		"plan upgrade action must target the public Pro pricing section");
	clipcoach::test::expect(
		brandMark != nullptr && !brandMark->pixmap().isNull() &&
			brandMark->width() == 22 && brandMark->height() == 22,
