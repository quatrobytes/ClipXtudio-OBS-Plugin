...
clipcoach::network::RemoteCommandPoller::Providers providers(bool pro, bool local, std::string token)
{
	...
	result.proActive = [pro] {
		return pro;
	};
	result.localCommandsEnabled = [local] {
		return local;
	};
	result.shuttingDown = [] {
		return false;
	};
	result.bearerToken = [token = std::move(token)] {
		return token;
	};
	result.heartbeat = [] {
...
int main(int argc, char **argv)
{
	QCoreApplication application(argc, argv);
	{
		...
		api.commandsResult.value = std::vector<clipcoach::remote::RemoteCommand>{
			{"123e4567-e89b-12d3-a456-426614174000", clipcoach::remote::RemoteCommandType::SaveVertical, 60,
			 10}};
		...
		assert(api.commandCalls == 1 &&
		       poller.status().connection == clipcoach::remote::RemoteConnectionState::Connected);
		poller.stop();
	}
	{
		FakeApi api;
		...
		dynamicProviders.bearerToken = [&token] {
			return token;
		};
		...
		assert(api.heartbeatTokens[0] == "old-token" && api.heartbeatTokens[1] == "new-token");
		api.pendingHeartbeats[0]({{}, {401, "LICENSE_TOKEN_REVOKED", "Old token revoked", false, true}});
		application.processEvents();
		assert(poller.status().connection != clipcoach::remote::RemoteConnectionState::Unauthorized);
		api.pendingHeartbeats[1](api.heartbeatResult);
		waitUntil([&] {
			return poller.status().connection == clipcoach::remote::RemoteConnectionState::Connected;
		});
		poller.stop();
	}
	{
		...
		poller.start();
		waitUntil([&] {
			return poller.status().connection == clipcoach::remote::RemoteConnectionState::Unauthorized;
		});
	...
	}
	{
		...
		poller.start();
		waitUntil([&] {
			return poller.status().connection == clipcoach::remote::RemoteConnectionState::Offline;
		});
