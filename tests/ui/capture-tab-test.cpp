...
public:
	...
	{
		savedCallback = std::move(callback);
	}
...
int main(int argc, char **argv)
{
	...
	clipcoach::test::expect(settings.save(configuredSettings, &error),
			       "capture duration fixture must save");
	...
	clipcoach::ui::MainDock dock(translator, &clips, &settings, nullptr, nullptr,
				    std::string{}, &vertical, nullptr, nullptr, nullptr,
				    nullptr, &proGates, nullptr);
	...
	clipcoach::test::expect(
		static_cast<bool>(replay.stateCallback),
	...
	auto *footerClip =
		dock.findChild<QPushButton *>(QStringLiteral("footerSaveClipButton"));
	auto *captureRemote =
	...
	auto *headerRemote = dock.findChild<QPushButton *>(
		QStringLiteral("headerRemoteOpenButton"));
	auto *remoteOverlay =
		dock.findChild<QWidget *>(QStringLiteral("remoteAuthOverlay"));
	auto *outputMode =
		dock.findChild<QComboBox *>(QStringLiteral("captureOutputModeCombo"));
	auto *outputDescription =
		dock.findChild<QLabel *>(QStringLiteral("captureOutputModeDescription"));
	auto *pendingDot =
		dock.findChild<QLabel *>(QStringLiteral("headerCapturePendingDot"));
	auto *insideObs = dock.findChild<QCheckBox *>(
		QStringLiteral("insideObsNotificationCheck"));
	auto *saveVertical =
		dock.findChild<QPushButton *>(QStringLiteral("saveVerticalClipButton"));
	auto *verticalRequirement =
		dock.findChild<QLabel *>(QStringLiteral("verticalProRequirementLabel"));
	auto *goVertical =
		dock.findChild<QPushButton *>(QStringLiteral("captureGoVerticalButton"));
	auto *bufferCard =
		dock.findChild<QWidget *>(QStringLiteral("captureBufferStatusCard"));
	auto *flowCard =
		dock.findChild<QWidget *>(QStringLiteral("captureRecommendedFlowCard"));
	auto *lastThumbnail =
		dock.findChild<QLabel *>(QStringLiteral("lastClipThumbnail"));
	...
	clipcoach::test::expect(goVertical != nullptr && bufferCard != nullptr &&
				       flowCard != nullptr && lastThumbnail != nullptr,
			       "Capture dashboard must expose navigation, buffer status, last clip and recommended flow");
	clipcoach::test::expect(
		outputMode != nullptr && outputMode->count() == 3 &&
			outputDescription != nullptr && outputDescription->wordWrap(),
		"Capture must expose one clear default format selector with readable guidance");
	outputMode->setCurrentIndex(
		static_cast<int>(clipcoach::CaptureOutputMode::Both));
	application.processEvents();
	clipcoach::test::expect(
		vertical.settings().outputMode == clipcoach::CaptureOutputMode::Both &&
	...
	outputMode->setCurrentIndex(
		static_cast<int>(clipcoach::CaptureOutputMode::Horizontal));
	application.processEvents();
	clipcoach::test::expect(
	...
	for (const auto *name :
	     {"startReplayBufferButtonInfoButton",
	      "markMomentButtonInfoButton",
	      "saveVerticalClipButtonInfoButton",
	      "captureGoVerticalButtonInfoButton"}) {
		auto *help =
			dock.findChild<QToolButton *>(QString::fromLatin1(name));
		clipcoach::test::expect(
	...
	clipcoach::test::expect(tabBar->currentIndex() == 0,
			       "Go to Vertical must preserve native tab navigation");
	tabBar->setCurrentIndex(1);

	clipcoach::test::expect(markMoment != nullptr && markMoment->isEnabled(),
			       "manual capture must be enabled while replay is active");
	clipcoach::test::expect(footerReplay != nullptr && footerReplay->isEnabled() &&
				       footerClip != nullptr && footerClip->isEnabled(),
	...
	clipcoach::test::expect(
		dock.findChild<QSpinBox *>(QStringLiteral("defaultDurationSpin")) == nullptr &&
			insideObs != nullptr,
		"capture duration must not be duplicated under Settings");
	clipcoach::test::expect(saveVertical != nullptr && saveVertical->isEnabled() &&
				       verticalRequirement != nullptr &&
				       verticalRequirement->isHidden(),
			       "an active Pro entitlement must unlock vertical capture");
	footerReplay->click();
	clipcoach::test::expect(replay.stopCalls == 1,
			       "global Replay action must delegate through ClipManager");
	footerClip->click();
	clipcoach::test::expect(replay.saveCalls == 1,
			       "global clip action must delegate through ClipManager");
	...
	clipcoach::test::expect(!notification->isHidden(),
			       "dock notification must become visible after save");
	clipcoach::test::expect(
		notification->text().endsWith(QStringLiteral("✓")),
		"saved notification must show a trailing check");
	clipcoach::test::expect(notification->maximumHeight() == 30,
			       "saved notification must use the compact height");
	clipcoach::test::expect(
		notification->sizePolicy().horizontalPolicy() ==
			QSizePolicy::Maximum,
		"saved notification must not stretch across the page");
	clipcoach::test::expect(
		headerStatus != nullptr &&
			headerStatus->text() == QString::fromUtf8(
				clipcoach::ui::strings::kHeaderBufferActive),
		"header status must stay focused on the concise active Buffer state");
	clipcoach::test::expect(
		clips.sessionClips().back().durationSeconds == 45,
	...
	dismissWait.exec();
	clipcoach::test::expect(notification->isHidden(),
			       "saved notification must auto-dismiss");
	clipcoach::test::expect(
		headerStatus->text() ==
	...
	application.processEvents();
	clipcoach::test::expect(
		notification->isHidden() && pendingDot->isHidden(),
	...
	onlineRemote.connection =
		clipcoach::remote::RemoteConnectionState::Connected;
	dock.setRemoteClipperStatus(onlineRemote);
	headerRemote->click();
	application.processEvents();
	clipcoach::test::expect(
		remoteSettingChanged && !requestedRemoteState &&
			!settings.settings().remoteCommandsEnabled &&
			remoteOverlay != nullptr && !remoteOverlay->isHidden() &&
			!headerRemote->isEnabled(),
	...
	pausedRemote.connection =
		clipcoach::remote::RemoteConnectionState::Paused;
	dock.setRemoteClipperStatus(pausedRemote);
	application.processEvents();
	clipcoach::test::expect(
		remoteOverlay->isHidden() && headerRemote->isEnabled() &&
			headerRemote->property("remoteState").toString() ==
				QStringLiteral("offline") &&
			headerRemote->toolTip() == QString::fromUtf8(
				clipcoach::ui::strings::kRemoteToggleTurnOn),
	...
	clipcoach::test::expect(
		remoteOverlay->isHidden() && headerRemote->isEnabled() &&
			...
			headerRemote->toolTip() == QString::fromUtf8(
				clipcoach::ui::strings::kRemoteToggleTurnOff),
