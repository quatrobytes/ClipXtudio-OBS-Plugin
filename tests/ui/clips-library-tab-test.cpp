...
int main(int argc, char **argv)
{
	...
	qputenv("CLIPXTUDIO_FFMPEG_PATH", QByteArrayLiteral(CLIPCOACH_TEST_FFMPEG_EXECUTABLE));
	...
	const auto clipPath = temporary.path().parent_path() / "clipcoach-ui-clip.mkv";
	QProcess mediaGenerator;
	mediaGenerator.start(QStringLiteral(CLIPCOACH_TEST_FFMPEG_EXECUTABLE),
			     {QStringLiteral("-hide_banner"), QStringLiteral("-loglevel"), QStringLiteral("error"),
			      QStringLiteral("-f"), QStringLiteral("lavfi"), QStringLiteral("-i"),
			      QStringLiteral("testsrc2=size=640x360:rate=30:duration=2"), QStringLiteral("-c:v"),
			      QStringLiteral("mpeg4"), QStringLiteral("-y"),
			      QString::fromStdString(clipPath.u8string())});
	clipcoach::test::expect(mediaGenerator.waitForFinished(10000) && mediaGenerator.exitCode() == 0 &&
	...
	const auto thumbnailPath = temporary.path().parent_path() / "clipcoach-ui-thumbnail.jpg";
	QImage thumbnail(320, 180, QImage::Format_RGB32);
	thumbnail.fill(QColor(QStringLiteral("#6D35D9")));
	clip.thumbnailPath = thumbnailPath;
	clipcoach::test::expect(thumbnail.save(QString::fromStdString(thumbnailPath.u8string())),
	...
	previousSession.endedAt = previousSession.startedAt + std::chrono::minutes(45);
	...
	clipcoach::test::expect(previousSessionPromise.get_future().get().success,
	...
	const auto previousClipPath = temporary.path().parent_path() / "clipcoach-ui-previous-clip.mkv";
	QFile previousClipFile(QString::fromStdString(previousClipPath.u8string()));
	clipcoach::test::expect(previousClipFile.open(QIODevice::WriteOnly) &&
	...
	library.storeClip(previousClip,
			  [&previousClipPromise](auto status) { previousClipPromise.set_value(std::move(status)); });
	...
	clipcoach::ui::MainDock dock(
		translator, nullptr, nullptr, &library, nullptr, QStringLiteral("session-test-001").toStdString(),
		nullptr, nullptr, nullptr, nullptr, nullptr, &gates, nullptr, {}, {},
		[&pendingCaptionCompletion, &pendingCaptionProgress](
			const clipcoach::ClipMetadata &, clipcoach::ui::CaptionGenerationProgressCallback progress,
	...
	application.processEvents();
	auto *thumbnailFrame = dock.findChild<QFrame *>(QStringLiteral("ClipThumbnail"));
	auto *thumbnailImage = dock.findChild<QLabel *>(QStringLiteral("clipThumbnailImage"));
	auto *details = dock.findChild<QWidget *>(QStringLiteral("clipDetails"));
	auto *trigger = dock.findChild<QLabel *>(QStringLiteral("TriggerBadge"));
	auto *previewAction = dock.findChild<QPushButton *>(QStringLiteral("clipPreviewButton"));
	auto *editAction = dock.findChild<QPushButton *>(QStringLiteral("clipQuickEditorButton"));
	auto *selectionForGeometry = dock.findChild<QCheckBox *>(QStringLiteral("clipSelectionCheck"));
	clipcoach::test::expect(thumbnailFrame != nullptr && thumbnailImage != nullptr &&
					!thumbnailImage->pixmap().isNull(),
				"persisted ClipCard must render the saved thumbnail image");
	clipcoach::test::expect(
		selectionForGeometry != nullptr && details != nullptr && previewAction != nullptr &&
			editAction != nullptr &&
			selectionForGeometry->mapTo(&dock, QPoint()).x() < thumbnailFrame->mapTo(&dock, QPoint()).x() &&
			thumbnailFrame->mapTo(&dock, QPoint()).x() < details->mapTo(&dock, QPoint()).x() &&
			details->mapTo(&dock, QPoint()).x() < previewAction->mapTo(&dock, QPoint()).x(),
	...
	clipcoach::test::expect(
		quickEditor != nullptr &&
			...
			quickEditor->findChild<QPushButton *>(QStringLiteral("quickEditorMarkCutStartButton")) !=
				nullptr &&
			quickEditor->findChild<QPushButton *>(QStringLiteral("quickEditorRemoveCutButton")) !=
				nullptr &&
			quickEditor->findChild<QPushButton *>(QStringLiteral("quickEditorUndoCutsButton")) != nullptr &&
			quickEditor->findChild<QPushButton *>(QStringLiteral("quickEditorSmartTrimButton")) !=
				nullptr &&
			quickEditor->findChild<QPushButton *>(QStringLiteral("quickEditorExportButton")) != nullptr &&
			quickEditor->findChild<QPushButton *>(QStringLiteral("quickEditorGenerateCaptionButton")) !=
				nullptr,
	...
	while (editorVideo != nullptr && editorVideo->pixmap().isNull() && previewTimeout.elapsed() < 5000)
		application.processEvents();
	...
	auto *quickCaptionButton =
		quickEditor->findChild<QPushButton *>(QStringLiteral("quickEditorGenerateCaptionButton"));
	quickCaptionButton->click();
	application.processEvents();
	auto *quickCaptionProgressDialog =
		quickEditor->findChild<QDialog *>(QStringLiteral("quickEditorCaptionProgressDialog"));
	auto *quickCaptionProgress =
		quickEditor->findChild<QProgressBar *>(QStringLiteral("quickEditorCaptionProgressBar"));
	clipcoach::test::expect(quickCaptionProgressDialog != nullptr && quickCaptionProgressDialog->isVisible() &&
	...
	application.processEvents();
	auto *quickCaptionStatus = quickEditor->findChild<QLabel *>(QStringLiteral("quickEditorCaptionProgressStatus"));
	clipcoach::test::expect(quickCaptionProgress->value() == 48 && quickCaptionStatus != nullptr &&
					quickCaptionStatus->text().contains(QStringLiteral("audio"),
									    Qt::CaseInsensitive),
	...
	pendingCaptionCompletion(
		{true, QStringLiteral("Caption listo #ClipXtudio"), {}, QStringLiteral("Caption listo #Shorts")});
	pendingCaptionCompletion = {};
	pendingCaptionProgress = {};
	application.processEvents();
	quickCaptionProgressDialog =
		quickEditor->findChild<QDialog *>(QStringLiteral("quickEditorCaptionProgressDialog"));
	clipcoach::test::expect(
	...
	while (editorRange->property("positionMilliseconds").toLongLong() == 0 && playbackTimeout.elapsed() < 1500)
		application.processEvents();
	clipcoach::test::expect(editorRange->property("positionMilliseconds").toLongLong() > 0,
				"playing the selection must advance the timeline playhead in real time");
	editorPlay->click();
	QMouseEvent seekPress(QEvent::MouseButtonPress, QPointF(editorRange->width() * 0.5, 45),
			      QPointF(editorRange->width() * 0.5, 45), Qt::LeftButton, Qt::LeftButton, Qt::NoModifier);
	QApplication::sendEvent(editorRange, &seekPress);
	clipcoach::test::expect(editorRange->property("positionMilliseconds").toLongLong() > 0,
				"clicking the timeline away from trim handles must seek the movable playhead");
	auto *splitButton = quickEditor->findChild<QPushButton *>(QStringLiteral("quickEditorMarkCutStartButton"));
	auto *deleteRangeButton = quickEditor->findChild<QPushButton *>(QStringLiteral("quickEditorRemoveCutButton"));
	splitButton->click();
	QMouseEvent secondSeek(QEvent::MouseButtonPress, QPointF(editorRange->width() * 0.75, 45),
			       QPointF(editorRange->width() * 0.75, 45), Qt::LeftButton, Qt::LeftButton,
			       Qt::NoModifier);
	QApplication::sendEvent(editorRange, &secondSeek);
	splitButton->click();
	QMouseEvent thirdSeek(QEvent::MouseButtonPress, QPointF(editorRange->width() * 0.25, 45),
			      QPointF(editorRange->width() * 0.25, 45), Qt::LeftButton, Qt::LeftButton, Qt::NoModifier);
	QApplication::sendEvent(editorRange, &thirdSeek);
	splitButton->click();
	QMouseEvent selectSecond(QEvent::MouseButtonPress, QPointF(editorRange->width() * 0.375, 45),
				 QPointF(editorRange->width() * 0.375, 45), Qt::LeftButton, Qt::LeftButton,
				 Qt::NoModifier);
	QApplication::sendEvent(editorRange, &selectSecond);
	clipcoach::test::expect(
		deleteRangeButton->isEnabled() && editorRange->property("selectedSegmentCount").toInt() == 1 &&
			!deleteRangeButton->icon().isNull() && !deleteRangeButton->text().contains(QLatin1Char('(')),
		"One selected timeline block must use the singular trash action");
	QMouseEvent addFourth(QEvent::MouseButtonPress, QPointF(editorRange->width() * 0.875, 45),
			      QPointF(editorRange->width() * 0.875, 45), Qt::LeftButton, Qt::LeftButton,
			      Qt::ControlModifier);
	QApplication::sendEvent(editorRange, &addFourth);
	clipcoach::test::expect(
		deleteRangeButton->isEnabled() && editorRange->property("selectedSegmentCount").toInt() == 2 &&
			!deleteRangeButton->icon().isNull() && deleteRangeButton->text().contains(QStringLiteral("2")),
		"Ctrl+click must select multiple non-adjacent timeline blocks for one deletion");
	deleteRangeButton->click();
	clipcoach::test::expect(editorRange->property("visibleSegmentCount").toInt() == 2 &&
					editorRange->property("removedRangeCount").toInt() == 2,
				"Delete must remove every selected block and ripple both retained blocks");
	QMouseEvent seekRetainedStart(QEvent::MouseButtonPress, QPointF(editorRange->width() * 0.1, 45),
				      QPointF(editorRange->width() * 0.1, 45), Qt::LeftButton, Qt::LeftButton,
				      Qt::NoModifier);
	QApplication::sendEvent(editorRange, &seekRetainedStart);
	const int segmentStartsBeforePlayback = editorRange->property("playbackSegmentStartCount").toInt();
	editorPlay->click();
	QElapsedTimer ripplePlaybackTimeout;
	ripplePlaybackTimeout.start();
	while (editorRange->property("playbackSegmentStartCount").toInt() <= segmentStartsBeforePlayback &&
	       ripplePlaybackTimeout.elapsed() < 1500)
		application.processEvents();
	clipcoach::test::expect(editorRange->property("playbackSegmentStartCount").toInt() >
	...
	clipcoach::test::expect(trigger != nullptr && trigger->text() == QStringLiteral("saca clip"),
				"ClipCard must show the phrase that triggered the clip instead of Manual");
	auto *historyScope = dock.findChild<QComboBox *>(QStringLiteral("clipsHistoryScope"));
	...
	while (freeCardCount() == 0 && freeTimeout.elapsed() < 2000)
		application.processEvents();
	clipcoach::test::expect(freeCardCount() == 1, "Free Clips tab must remain limited to the current session");
	for (const auto *name :
	     {"clipFavoriteButton", "clipPreviewButton", "clipExportVerticalButton", "clipCaptionButton",
	      "clipSubtitlesButton", "clipOpenFolderButton", "clipDeleteButton"}) {
	...
	auto *captionAction = dock.findChild<QPushButton *>(QStringLiteral("clipCaptionButton"));
	auto *subtitlesAction = dock.findChild<QPushButton *>(QStringLiteral("clipSubtitlesButton"));
	clipcoach::test::expect(captionAction != nullptr && captionAction->isEnabled(),
				"Pro clip cards must enable caption generation when a generator is connected");
	clipcoach::test::expect(subtitlesAction != nullptr && !subtitlesAction->isEnabled() &&
					subtitlesAction->toolTip().contains(QStringLiteral("Próximamente")),
				"subtitle action must remain visibly unavailable with a Próximamente tooltip");
	captionAction->click();
	auto *captionOverlay = dock.findChild<QFrame *>(QStringLiteral("captionBusyOverlay"));
	auto *captionProgress = dock.findChild<QProgressBar *>(QStringLiteral("captionBusyProgress"));
	clipcoach::test::expect(captionOverlay != nullptr && captionOverlay->isVisible() &&
	...
	clipcoach::test::expect(captionBusyCard != nullptr && captionBusyCard->minimumWidth() >= 480 &&
					captionBusyLabel != nullptr && captionBusyLabel->wordWrap() &&
					captionBusyLabel->minimumHeight() >= 44 && captionBusySpinner != nullptr &&
					captionProgressTimer != nullptr && captionProgressTimer->interval() == 250,
	...
	pendingCaptionProgress({47, QStringLiteral("Analizando video y audio"), 38});
	application.processEvents();
	clipcoach::test::expect(captionProgress->value() == 47 && captionBusyPercent != nullptr &&
					captionBusyPercent->text() == QStringLiteral("47%") &&
					captionBusyLabel->text() == QStringLiteral("Analizando video y audio") &&
					captionBusyEta != nullptr &&
					captionBusyEta->text().contains(QStringLiteral("38")),
				"caption overlay must show the reported percentage, media stage and remaining time");
	pendingCaptionProgress({82, QStringLiteral("Generando el caption con IA"), 0});
	for (int tick = 0; tick < 24; ++tick)
		QMetaObject::invokeMethod(captionProgressTimer, "timeout", Qt::DirectConnection);
	clipcoach::test::expect(
		captionBusyPercent->text() == QStringLiteral("82%") && !captionBusySpinner->pixmap().isNull() &&
			captionBusyLabel->text().contains(QStringLiteral("casi"), Qt::CaseInsensitive) &&
			captionBusyEta->text().contains(QStringLiteral("sigue"), Qt::CaseInsensitive),
		"a stalled percentage must keep an animated spinner and rotate to reassuring active-work copy");
	QDir().mkpath(QStringLiteral(CLIPCOACH_UI_ARTIFACT_DIR));
	clipcoach::test::expect(dock.grab().save(QStringLiteral(CLIPCOACH_UI_ARTIFACT_DIR "/caption-progress.png")),
	...
	auto *captionText = captionDialog != nullptr
				    ? captionDialog->findChild<QPlainTextEdit *>(QStringLiteral("captionText"))
				    : nullptr;
	auto *shortsText =
		captionDialog != nullptr
			? captionDialog->findChild<QPlainTextEdit *>(QStringLiteral("youtubeShortsCaptionText"))
			: nullptr;
	auto *copySocial = captionDialog != nullptr
				   ? captionDialog->findChild<QPushButton *>(QStringLiteral("copyCaptionButton"))
				   : nullptr;
	auto *copyShorts =
		captionDialog != nullptr
			? captionDialog->findChild<QPushButton *>(QStringLiteral("copyYoutubeShortsCaptionButton"))
			: nullptr;
	clipcoach::test::expect(captionDialog != nullptr, "caption generation must create a result dialog");
	clipcoach::test::expect(captionDialog->isVisible(), "caption generation result dialog must be visible");
	clipcoach::test::expect(captionOverlay->isHidden(),
				"caption processing overlay must close before presenting the final result");
	clipcoach::test::expect(captionText != nullptr, "caption result dialog must expose copyable text");
	clipcoach::test::expect(
		captionText->toPlainText().contains(QStringLiteral("Caption sugerido")) &&
			captionText->toPlainText().count(QLatin1Char('#')) == 5 &&
			captionText->toPlainText().split(QStringLiteral("\n\n"), Qt::SkipEmptyParts).size() == 2,
	...
	const auto normalizedSocial = clipcoach::ui::formatSocialCaption(
		...
		{QStringLiteral("#Uno"), QStringLiteral("#Dos"), QStringLiteral("#Tres"), QStringLiteral("#Cuatro"),
		 QStringLiteral("#Cinco")});
	const auto normalizedParts = normalizedSocial.split(QStringLiteral("\n\n"), Qt::SkipEmptyParts);
	clipcoach::test::expect(
		normalizedParts.size() == 2 &&
			normalizedParts.front().toUcs4().size() <= clipcoach::ui::kSocialCaptionMaximumBodyCharacters &&
	...
	clipcoach::test::expect(
		shortsText != nullptr && shortsText->toPlainText().toUcs4().size() <= 100 &&
			shortsText->toPlainText().contains(QLatin1Char('#')) && copySocial != nullptr &&
			copyShorts != nullptr && copySocial->geometry().top() >= captionText->geometry().bottom() &&
	...
	clipcoach::test::expect(captionDialog->grab().save(
					QStringLiteral(CLIPCOACH_UI_ARTIFACT_DIR "/caption-result-two-platforms.png")),
				"two-platform caption dialog preview must be saved");
	copySocial->click();
	clipcoach::test::expect(QApplication::clipboard()->text() == captionText->toPlainText(),
				"social copy action must copy only the TikTok, Instagram and Facebook caption");
	copyShorts->click();
	clipcoach::test::expect(QApplication::clipboard()->text() == shortsText->toPlainText(),
				"YouTube copy action must copy only the Shorts caption");
	captionDialog->close();
	application.processEvents();
	QElapsedTimer reloadTimeout;
	reloadTimeout.start();
	while ((reloadTimeout.elapsed() < 100 || !loading->isHidden() || persistedCardCount() < 1) &&
	       reloadTimeout.elapsed() < 2000)
		application.processEvents();
	clipcoach::test::expect(loading->isHidden() && persistedCardCount() >= 1,
	...
	clipcoach::test::expect(selection != nullptr && batchExport != nullptr && batchDelete != nullptr,
	...
	clipcoach::test::expect(selection->isChecked() && batchDelete->isEnabled(),
	...
	deleteTimeout.start();
	while (std::filesystem::exists(clipPath) && std::filesystem::exists(previousClipPath) &&
	       deleteTimeout.elapsed() < 2000)
		application.processEvents();
	clipcoach::test::expect(!std::filesystem::exists(clipPath) || !std::filesystem::exists(previousClipPath),
				"bulk deletion must forward selected IDs and delete the media file");

	std::promise<clipcoach::storage::RepositoryResult<std::vector<clipcoach::ClipMetadata>>> deletedListPromise;
	library.listRecentAsync(10, [&deletedListPromise](auto result) {
		deletedListPromise.set_value(std::move(result));
	});
	const auto deletedList = deletedListPromise.get_future().get();
	clipcoach::test::expect(deletedList.success && deletedList.value.size() == 1,
