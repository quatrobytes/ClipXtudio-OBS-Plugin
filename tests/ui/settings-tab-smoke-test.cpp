...
int main(int argc, char **argv)
{
	...
	replayBridge.replayEncoders = [] {
		return std::vector<clipcoach::ui::ReplayEncoderOption>{{"nvenc", "NVIDIA NVENC H.264", true},
								       {"x264", "Software x264", false}};
	};
	replayBridge.replayProfile = [] {
		return clipcoach::ui::ReplayProfileSettings{"Advanced", "x264", "Software x264", false, true};
	};
	replayBridge.applyReplayProfile = [&replayProfileApplied](const std::string &encoder, bool enabled) {
		replayProfileApplied = encoder == "nvenc" && enabled;
		return clipcoach::ui::ReplayProfileApplyResult{true, true, "saved"};
	};
	replayBridge.restartObs = [&restartRequested](std::string *) {
		restartRequested = true;
		return true;
	};
	clipcoach::ui::SettingsTab tab(translator, &manager, nullptr, nullptr, nullptr, replayBridge);
	tab.resize(520, 820);
	tab.show();
	application.processEvents();

	for (const auto *name :
	     {"settingsGeneralSection", "settingsProfileSection", "settingsReplayPerformanceSection",
	      "settingsPathsSection", "settingsFileNamesSection", "settingsNotificationsSection",
	      "settingsHotkeysSection", "settingsExportSection", "settingsIntegrationsSection", "settingsAiSection",
	...
	auto *sectionsContainer = tab.findChild<QWidget *>(QStringLiteral("settingsSectionsContainer"));
	auto *sectionsGrid = sectionsContainer != nullptr ? qobject_cast<QGridLayout *>(sectionsContainer->layout())
							  : nullptr;
	...
	for (const auto *name :
		...
		clipcoach::test::expect(section != nullptr &&
	...
	auto *remoteStatusGrid = tab.findChild<QWidget *>(QStringLiteral("remoteClipperStatusGrid"));
	const auto remoteStatusTiles =
		remoteStatusGrid != nullptr
			? remoteStatusGrid->findChildren<QWidget *>(QStringLiteral("RemoteClipperStatusTile"),
								    Qt::FindDirectChildrenOnly)
			: QList<QWidget *>();
	clipcoach::test::expect(
		remoteStatusGrid != nullptr && remoteStatusTiles.size() == 3 &&
			qobject_cast<QGridLayout *>(remoteStatusGrid->layout()) != nullptr &&
			qobject_cast<QGridLayout *>(remoteStatusGrid->layout())->itemAtPosition(1, 0) != nullptr &&
			qobject_cast<QGridLayout *>(remoteStatusGrid->layout())->itemAtPosition(2, 0) != nullptr &&
			tab.findChild<QLabel *>(QStringLiteral("remoteClipperConnectionStatus"))->text() ==
				translator("Settings.RemoteClipper.Offline") &&
			!translator("Settings.RemoteClipper.Description").startsWith(QStringLiteral("Settings.")),
	...
	auto *remoteAuthenticate = tab.findChild<QPushButton *>(QStringLiteral("remoteClipperAuthenticateButton"));
	clipcoach::test::expect(remoteAuthenticate != nullptr && remoteAuthenticate->isHidden() &&
					!remoteAuthenticate->isEnabled() &&
					remoteAuthenticate->text() == translator("Settings.RemoteClipper.Authenticate"),
	...
	auto *exportProfile = tab.findChild<QPushButton *>(QStringLiteral("exportSettingsProfileButton"));
	auto *importProfile = tab.findChild<QPushButton *>(QStringLiteral("importSettingsProfileButton"));
	auto *openSetup = tab.findChild<QPushButton *>(QStringLiteral("openInitialSetupButton"));
	auto *profileDescription = tab.findChild<QLabel *>(QStringLiteral("settingsProfileDescription"));
	auto *pluginLanguage = tab.findChild<QComboBox *>(QStringLiteral("pluginLanguageCombo"));
	auto *verticalResolution = tab.findChild<QComboBox *>(QStringLiteral("verticalResolutionCombo"));
	...
	if (openSetup != nullptr)
		openSetup->click();
	clipcoach::test::expect(dockToggle != nullptr && exportProfile != nullptr && importProfile != nullptr &&
					openSetup != nullptr && setupRequested && profileDescription != nullptr &&
					profileDescription->wordWrap() && pluginLanguage != nullptr &&
					pluginLanguage->count() == 3 && verticalResolution != nullptr &&
					verticalResolution->count() == 6 && templateEdit != nullptr,
	...
	clipcoach::test::expect(manager.save(changed, &error), "settings profile fixture must be mutable");
	bool profileApplied = false;
	tab.setAppliedCallback([&profileApplied](const clipcoach::Settings &before, const clipcoach::Settings &after) {
		profileApplied = before.voiceTriggerPhrasesCsv != after.voiceTriggerPhrasesCsv;
	});
	clipcoach::test::expect(tab.importProfileFrom(profilePath, &error) && profileApplied &&
	...
	clipcoach::test::expect(tab.findChild<QWidget *>(QStringLiteral("settingsCaptureSection")) == nullptr &&
	...
	clipcoach::test::expect(
		verticalResolution->findData(static_cast<int>(clipcoach::VerticalResolution::Portrait4320)) >= 0,
		"Settings must expose the 8K vertical preset using enum-backed data");
	auto *replayEncoder = tab.findChild<QComboBox *>(QStringLiteral("settingsReplayEncoderCombo"));
	auto *replayEnabled = tab.findChild<QCheckBox *>(QStringLiteral("settingsReplayEnabledCheck"));
	auto *replayApply = tab.findChild<QPushButton *>(QStringLiteral("settingsReplayApplyButton"));
	auto *replayStatus = tab.findChild<QLabel *>(QStringLiteral("settingsReplayProfileStatus"));
	clipcoach::test::expect(replayEncoder != nullptr && replayEncoder->count() == 2 &&
					...
					replayEncoder->view()->minimumWidth() >=
						QFontMetrics(replayEncoder->view()->font())
							.horizontalAdvance(replayEncoder->itemText(0)) &&
	...
	replayEncoder->setCurrentIndex(replayEncoder->findData(QStringLiteral("nvenc")));
	replayApply->click();
	application.processEvents();
	clipcoach::test::expect(replayProfileApplied && replayStatus->text() == translator("Vertical.Replay.Restart"),
				"applying a Replay Buffer encoder from Settings must use the OBS bridge");
	auto *restartDialog = tab.findChild<QMessageBox *>(QStringLiteral("settingsReplayRestartDialog"));
	auto *restartButton = tab.findChild<QPushButton *>(QStringLiteral("settingsReplayRestartNowButton"));
	clipcoach::test::expect(restartDialog != nullptr && restartButton != nullptr,
				"Replay Buffer changes must show a native restart confirmation in Settings");
	restartButton->click();
	application.processEvents();
	clipcoach::test::expect(restartRequested &&
					replayStatus->text() == translator("Vertical.Replay.RestartScheduled"),
	...
	auto *replayEncoderHelp = tab.findChild<QToolButton *>(QStringLiteral("settingsReplayEncoderComboInfoButton"));
	auto *replaySectionHelp =
		tab.findChild<QToolButton *>(QStringLiteral("settingsReplayPerformanceSectionInfoButton"));
	const auto pathRows = tab.findChildren<QWidget *>(QStringLiteral("SettingsPathRow"));
	clipcoach::test::expect(replayEncoderHelp != nullptr && !replayEncoderHelp->toolTip().isEmpty() &&
					!replayEncoderHelp->toolTip().startsWith(QStringLiteral("Settings.")) &&
					replaySectionHelp != nullptr && !replaySectionHelp->toolTip().isEmpty() &&
					pathRows.size() == 3,
	...
	auto *clipPathContainer = tab.findChild<QWidget *>(QStringLiteral("clipDirectoryEditContainer"));
	auto *clipPathEdit = tab.findChild<QLineEdit *>(QStringLiteral("clipDirectoryEdit"));
	auto *clipPathChange = tab.findChild<QPushButton *>(QStringLiteral("clipDirectoryEditChange"));
	auto *clipPathOpen = tab.findChild<QPushButton *>(QStringLiteral("clipDirectoryEditOpen"));
	clipcoach::test::expect(clipPathContainer != nullptr && clipPathEdit != nullptr && clipPathChange != nullptr &&
					clipPathOpen != nullptr && clipPathContainer->layout() != nullptr,
	...
	clipcoach::test::expect(aiEnabled != nullptr && aiConsent != nullptr && aiLanguage != nullptr &&
					...
					sendSuggestion != nullptr &&
					reportBug->text() == QStringLiteral("REPORTAR UN ERROR"),
				"Free must see AI controls and privacy notice in a locked state");
	clipcoach::test::expect(tab.findChild<QWidget *>(QStringLiteral("settingsAboutUpdatesSection")) == nullptr,
	...
	tab.resize(900, 820);
	application.processEvents();
	clipcoach::test::expect(widthHint->isHidden(), "workspace hint must disappear when Settings has enough width");
	for (auto *row : formRows) {
		auto *box = qobject_cast<QBoxLayout *>(row->layout());
	...
	clipcoach::test::expect(sectionsGrid->itemAtPosition(0, 0) != nullptr &&
					sectionsGrid->itemAtPosition(0, 1) != nullptr,
				"wide Settings must use multiple columns instead of an endless vertical list");
	clipcoach::test::expect(clipcoach::network::QtUpdateChecker::isVersionNewer(QStringLiteral("1.2.0"),
	...
	verticalResolution->setCurrentIndex(
		verticalResolution->findData(static_cast<int>(clipcoach::VerticalResolution::Portrait4320)));
	nativeManagedHotkey->setKeySequence(QKeySequence(QStringLiteral("Ctrl+Alt+K")));
	QMetaObject::invokeMethod(nativeManagedHotkey, "editingFinished", Qt::DirectConnection);
	application.processEvents();
	clipcoach::test::expect(
		!manager.settings().openDockAtStartup && manager.settings().defaultDurationSeconds == 30 &&
			manager.settings().language == "es-ES" &&
			manager.settings().verticalResolution == clipcoach::VerticalResolution::Portrait4320 &&
			manager.settings().verticalWidth == 4320 && manager.settings().verticalHeight == 7680,
		"settings and explicit Spanish UI language must apply and persist immediately");
	clipcoach::test::expect(manager.settings().markMomentHotkey == "Ctrl+Alt+K",
	...
	clipcoach::test::expect(
		reloaded.load(&error) && !reloaded.settings().openDockAtStartup &&
			reloaded.settings().defaultDurationSeconds == 30 && reloaded.settings().language == "es-ES" &&
			reloaded.settings().verticalResolution == clipcoach::VerticalResolution::Portrait4320 &&
			reloaded.settings().verticalWidth == 4320 && reloaded.settings().verticalHeight == 7680,
		"settings changed in the UI must survive reload");
	clipcoach::test::expect(reloaded.settings().markMomentHotkey == "Ctrl+Alt+K",
