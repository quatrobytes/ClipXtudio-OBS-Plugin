...
{
	return QWheelEvent(QPointF(10, 10), QPointF(10, 10), QPoint(0, 0), QPoint(0, 120), Qt::NoButton, Qt::NoModifier,
...
int main(int argc, char **argv)
{
	...
	clipcoach::test::expect(slider.value() == 50, "Mouse wheel must not change a slider value");
	clipcoach::test::expect(!sliderWheel.isAccepted(),
	...
	clipcoach::test::expect(spin.value() == 50, "Mouse wheel must not change a spin box value");
	clipcoach::test::expect(!spinWheel.isAccepted(),
	...
	clipcoach::test::expect(combo.currentIndex() == 0, "Mouse wheel must not change a combo box selection");
	clipcoach::test::expect(!comboWheel.isAccepted(),
