...
int main(int argc, char **argv)
{
	...
	auto *pendingDot = verticalClipCard.findChild<QLabel *>(
		QStringLiteral("clipPendingDot"));
	auto *editorButton = verticalClipCard.findChild<QPushButton *>(
		QStringLiteral("clipQuickEditorButton"));
	clipcoach::test::expect(
		pendingDot != nullptr && !pendingDot->isHidden() &&
			pendingDot->toolTip() == QStringLiteral("Processing clip"),
		"a processing clip card must expose a visible accessible pending dot");
	clipcoach::test::expect(
		editorButton != nullptr && editorButton->toolTip() == QStringLiteral("Edit and trim"),
	...
	clipcoach::test::expect(planAction->isHidden(),
			       "a status card action must be hideable for active Pro plans");
	...
	auto *contextHelp =
		toggleRow.findChild<QToolButton *>(QStringLiteral("triggerInfoButton"));
	clipcoach::test::expect(
		contextHelp != nullptr &&
			contextHelp->toolTip() == QStringLiteral("Description") &&
			contextHelp->cursor().shape() == Qt::PointingHandCursor &&
			contextHelp->text() == QStringLiteral("i") &&
			contextHelp->isCheckable(),
	...
	clipcoach::test::expect(
		helpPopup != nullptr &&
			helpPopup->objectName() == QStringLiteral("ContextHelpPopup") &&
	...
	clipcoach::test::expect(
		!contextHelp->isChecked() &&
			(helpPopup == nullptr || !helpPopup->isVisible()),
	...
	clipcoach::test::expect(
		!verticalClipCard
			 .findChild<QPushButton *>(
				 QStringLiteral("clipExportVerticalButton"))
			 ->isEnabled(),
	...
	for (auto *button : verticalClipCard.findChildren<QPushButton *>()) {
		...
		++compactActions;
		clipcoach::test::expect(
			button->maximumHeight() <= 34,
			"clip actions must remain compact");
		clipcoach::test::expect(
			button->geometry().right() <
				verticalClipCard.width(),
			"every clip action must remain inside the card");
		if (actionTop < 0)
			actionTop = button->geometry().top();
		else
			clipcoach::test::expect(
				button->geometry().top() == actionTop,
	...
	auto *deleteAction = verticalClipCard.findChild<QPushButton *>(
		QStringLiteral("clipDeleteButton"));
	clipcoach::test::expect(
		deleteAction != nullptr &&
			deleteAction->property("destructiveAction").toBool(),
		"ClipCard must expose a clearly destructive delete action");
	auto *selection =
		verticalClipCard.findChild<QCheckBox *>(
			QStringLiteral("clipSelectionCheck"));
	auto *thumbnail =
		verticalClipCard.findChild<QWidget *>(QStringLiteral("ClipThumbnail"));
	auto *details =
		verticalClipCard.findChild<QWidget *>(QStringLiteral("clipDetails"));
	clipcoach::test::expect(
		selection != nullptr && thumbnail != nullptr && details != nullptr &&
			selection->geometry().left() <
				thumbnail->geometry().left() &&
			thumbnail->geometry().left() < details->geometry().left() &&
			selection->geometry().left() <
				verticalClipCard.width() / 3,
		"selection, thumbnail and metadata must remain grouped and aligned on the left");
	for (auto *button : verticalClipCard.findChildren<QPushButton *>()) {
		if (button->property("clipAction").toBool())
			clipcoach::test::expect(!button->icon().isNull() &&
	...
	clipcoach::test::expect(
		upgradeButton != nullptr &&
			upgradeButton->property("externalUrl")
				.toString()
				.endsWith(QStringLiteral("#pricing")),
		"UpgradeBanner must link every Pro action to public pricing");

	QMessageBox compactUpdateDialog(
		QMessageBox::Information, QStringLiteral("ClipXtudio is up to date"),
	...
	clipcoach::ui::tokens::configureCompactUpdateDialog(
		&compactUpdateDialog, 360);
	compactUpdateDialog.show();
	application.processEvents();
	auto *updateIcon = compactUpdateDialog.findChild<QLabel *>(
		QStringLiteral("qt_msgboxex_icon_label"));
	auto *updateText = compactUpdateDialog.findChild<QLabel *>(
		QStringLiteral("qt_msgbox_label"));
	clipcoach::test::expect(
		updateIcon != nullptr && updateIcon->width() <= 32,
		"the update dialog icon must not reserve a wide empty column");
	clipcoach::test::expect(
		updateText != nullptr && updateText->width() <= 360,
	...
	QMessageBox remoteAuthenticationDialog(
		...
		QStringLiteral("No fue posible renovar o validar Remote Clipper. Comprueba tu conexión y vuelve a intentarlo."),
		QMessageBox::Ok);
	remoteAuthenticationDialog.setObjectName(
		QStringLiteral("remoteAuthenticationResultDialog"));
	remoteAuthenticationDialog.addButton(
		QStringLiteral("Abrir mi cuenta"), QMessageBox::ActionRole);
	remoteAuthenticationDialog.setStyleSheet(
		clipcoach::ui::tokens::styleSheet());
	clipcoach::ui::tokens::configureRemoteAuthenticationDialog(
		&remoteAuthenticationDialog);
	remoteAuthenticationDialog.show();
	application.processEvents();
	auto *remoteText = remoteAuthenticationDialog.findChild<QLabel *>(
		QStringLiteral("qt_msgbox_label"));
	clipcoach::test::expect(
		remoteText != nullptr && remoteText->wordWrap() &&
			remoteText->width() == 520 && remoteText->height() >= 76,
		"the Remote Clipper authentication message must wrap without being clipped");
	clipcoach::test::expect(
		remoteAuthenticationDialog.width() >= 700 &&
			remoteAuthenticationDialog.height() >= 250,
	...
	clipcoach::test::expect(
		remoteOkButton != nullptr && remoteOkButton->width() == 80,
