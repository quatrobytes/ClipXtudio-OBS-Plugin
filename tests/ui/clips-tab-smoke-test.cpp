...
void expectNoOverlap(const std::vector<QWidget *> &widgets, QWidget *ancestor, const char *message)
{
	for (std::size_t left = 0; left < widgets.size(); ++left) {
		for (std::size_t right = left + 1; right < widgets.size(); ++right) {
			clipcoach::test::expect(!relativeRect(widgets[left], ancestor)
							 .intersects(relativeRect(widgets[right], ancestor)),
						message);
		}
	}
}
...
int main(int argc, char **argv)
{
	...
	clipcoach::test::expect(tab.findChild<QWidget *>(QStringLiteral("clipsSummaryCards")) != nullptr &&
	...
	const auto *metricsLayout = tab.findChild<QGridLayout *>(QStringLiteral("clipsMetricsRow"));
	const auto *filtersLayout = tab.findChild<QGridLayout *>(QStringLiteral("clipsFiltersGrid"));
	const auto *exportLayout = tab.findChild<QGridLayout *>(QStringLiteral("clipsExportGrid"));
	const auto *sort = tab.findChild<QComboBox *>(QStringLiteral("clipsSort"));
	clipcoach::test::expect(metricsLayout != nullptr && filtersLayout != nullptr && exportLayout != nullptr &&
					sort != nullptr && exportLayout->indexOf(sort) >= 0,
	...
	clipcoach::test::expect(exportMargins.top() == clipcoach::ui::tokens::kSpaceXs &&
					exportMargins.bottom() == clipcoach::ui::tokens::kSpaceXs,
				"library toolbar must reserve equal top and bottom margins");
	for (const auto *name : {"batchExportButton", "cancelExportButton", "batchDeleteButton"}) {
		auto *button = tab.findChild<QPushButton *>(QString::fromUtf8(name));
		clipcoach::test::expect(button != nullptr &&
	...
	clipcoach::test::expect(exportHost != nullptr, "library global actions must remain inside the toolbar host");
	for (const auto *name :
	...
	std::vector<QWidget *> metricCards{tab.findChild<QFrame *>(QStringLiteral("clipsTotalCard")),
	...
	for (const auto *name : {"clipsFilterAll", "clipsFilterFavorites", "clipsFilterVertical", "clipsFilterPending"})
	...
	clipcoach::test::expect(
		tab.findChild<QWidget *>(QStringLiteral("clipsOverviewHost"))->property("layoutMode").toString() ==
	...
	clipcoach::test::expect(relativeRect(metricCards.front(), &tab).top() ==
