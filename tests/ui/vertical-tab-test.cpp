...
public:
	...
	{
		return {"Shure Mic", "USB Microphone"};
	}
...
int main(int argc, char **argv)
{
	...
	clipcoach::test::expect(
		vertical.setObsSelection("ClipXtudio Vertical", {}, &error) &&
	...
	bridge.createPreview = [](QWidget *parent) { return new QWidget(parent); };
	...
	clipcoach::ui::VerticalTab liveTab(
		[](const char *key) { return QString::fromUtf8(key); }, &vertical, bridge);
	bool startReplayRequested = false;
	bool saveClipRequested = false;
	liveTab.setCaptureActions(
		[&startReplayRequested] { startReplayRequested = true; },
	...
	if (qEnvironmentVariableIsSet("CLIPX_RENDER_ONLY")) {
		...
		clipcoach::test::expect(
			liveTab.grab().save(QStringLiteral(
				CLIPCOACH_UI_ARTIFACT_DIR "/vertical-layout-narrow.png")),
			"Compact Vertical layout preview must be saved");
		liveTab.resize(1400, 900);
		application.processEvents();
		clipcoach::test::expect(
			liveTab.grab().save(QStringLiteral(
				CLIPCOACH_UI_ARTIFACT_DIR "/vertical-layout-wide.png")),
			"Vertical layout preview must be saved");
		return clipcoach::test::pass("vertical-layout-render");
	}
	auto *pageHeader =
		liveTab.findChild<QWidget *>(QStringLiteral("verticalPageHeader"));
	auto *previewCard =
		liveTab.findChild<QWidget *>(QStringLiteral("verticalPreviewCard"));
	auto *compositionHeading = liveTab.findChild<QWidget *>(
		QStringLiteral("verticalCompositionHeading"));
	auto *canvasHeading = liveTab.findChild<QWidget *>(
		QStringLiteral("verticalCanvasHeading"));
	auto *positionHeading = liveTab.findChild<QWidget *>(
		QStringLiteral("verticalPositionHeading"));
	auto *compositionCard = liveTab.findChild<QWidget *>(
		QStringLiteral("verticalCompositionCard"));
	auto *framingControls = liveTab.findChild<QWidget *>(
		QStringLiteral("verticalFramingControls"));
	auto *startReplay = liveTab.findChild<QPushButton *>(
		QStringLiteral("verticalStartReplayButton"));
	auto *saveClip = liveTab.findChild<QPushButton *>(
		QStringLiteral("verticalSaveClipButton"));
	auto *createScene = liveTab.findChild<QPushButton *>(
		QStringLiteral("verticalCreateSceneButton"));
	auto *activeBadge = liveTab.findChild<QLabel *>(
		QStringLiteral("verticalActiveBadge"));
	auto *autoSaved = liveTab.findChild<QLabel *>(
		QStringLiteral("verticalAutoSavedLabel"));
	auto *savedNow = liveTab.findChild<QLabel *>(
		QStringLiteral("verticalSavedNowLabel"));
	auto *compactBar = liveTab.findChild<QWidget *>(
		QStringLiteral("verticalCompactActionBar"));
	auto *compactReplay = liveTab.findChild<QPushButton *>(
		QStringLiteral("verticalCompactReplayButton"));
	auto *compactSave = liveTab.findChild<QPushButton *>(
		QStringLiteral("verticalCompactSaveButton"));
	auto *compactCreate = liveTab.findChild<QPushButton *>(
		QStringLiteral("verticalCompactCreateSceneButton"));
	auto *previewOnly = liveTab.findChild<QPushButton *>(
		QStringLiteral("verticalPreviewOnlyButton"));
	clipcoach::test::expect(
		pageHeader != nullptr && previewCard != nullptr && compositionCard != nullptr &&
			framingControls != nullptr &&
			compositionHeading != nullptr && canvasHeading != nullptr &&
			positionHeading != nullptr,
		"Vertical redesign must expose a clear header, preview card and purpose-based control groups");
	clipcoach::test::expect(
		startReplay != nullptr && startReplay->isEnabled() &&
			saveClip != nullptr && !saveClip->isEnabled() &&
			createScene != nullptr &&
			activeBadge != nullptr && autoSaved != nullptr &&
			savedNow != nullptr,
		"Vertical primary actions must reflect the inactive Replay Buffer state");
	clipcoach::test::expect(
		pageHeader->isHidden() && compactBar != nullptr &&
			compactBar->isVisible() && compactReplay != nullptr &&
			compactSave != nullptr && compactCreate != nullptr &&
			previewOnly != nullptr && previewOnly->isVisible() &&
			compactReplay->text().isEmpty() &&
			compactSave->text().isEmpty() &&
			compactCreate->text().isEmpty() &&
	...
	clipcoach::test::expect(
		startReplay->minimumWidth() == 194 &&
			startReplay->maximumWidth() == 194 &&
			saveClip->minimumWidth() == 142 &&
			saveClip->maximumWidth() == 142 &&
			activeBadge->minimumWidth() == 132 &&
			activeBadge->maximumWidth() == 132 &&
	...
	application.processEvents();
	clipcoach::test::expect(
		pageHeader->isVisible() && compactBar->isHidden() &&
			previewOnly->isVisible() &&
	...
	compactSave->click();
	clipcoach::test::expect(
		startReplayRequested && saveClipRequested &&
			activeBadge->property("verticalActive").toBool(),
		"compact Vertical actions must reuse capture callbacks and show the live Replay Buffer state");
	clipcoach::test::expect(
		liveTab.findChildren<QPushButton *>(
			QStringLiteral("verticalCreateSceneButton"))
				.size() == 1,
		"Vertical redesign must keep one bound create-scene action");
	auto *sceneSelector =
		liveTab.findChild<QComboBox *>(QStringLiteral("verticalObsSceneCombo"));
	auto *sourceSelector =
		liveTab.findChild<QComboBox *>(QStringLiteral("verticalObsSourceCombo"));
	auto *zoom = liveTab.findChild<QSpinBox *>(QStringLiteral("verticalZoom"));
	clipcoach::test::expect(sceneSelector != nullptr && sourceSelector != nullptr &&
				       sceneSelector->count() == 4,
			       "Vertical tab must list live OBS scenes through the adapter boundary");
	clipcoach::test::expect(
		sceneSelector->findData(QStringLiteral("ClipXtudio Vertical")) >= 0 &&
	...
	clipcoach::test::expect(
		vertical.selectedSceneName() == "ClipXtudio Vertical" &&
			vertical.settings().zoomPercent == 160 &&
			vertical.settings().panXPercent == -67 &&
			vertical.settings().panYPercent == -100,
	...
	zoom->setValue(135);
	application.processEvents();
	clipcoach::test::expect(livePreviewUpdated &&
				       vertical.selectedSceneName() == "Gameplay" &&
	...
	delayedBridge.scenes = [&delayedScenes] { return delayedScenes; };
	...
	clipcoach::ui::VerticalTab restartedTab(
		[](const char *key) { return QString::fromUtf8(key); },
		&restartedVertical, delayedBridge);
	auto *restartedSceneSelector = restartedTab.findChild<QComboBox *>(
		QStringLiteral("verticalObsSceneCombo"));
	clipcoach::test::expect(
	...
	clipcoach::test::expect(
		liveTab.findChild<QWidget *>(
			QStringLiteral("verticalReplayProfileSection")) == nullptr &&
			liveTab.findChild<QComboBox *>(
				QStringLiteral("verticalReplayEncoderCombo")) == nullptr,
		"Vertical must stay focused on 9:16 composition; Replay Buffer performance belongs in Settings");
	auto *responsivePreview =
		liveTab.findChild<QWidget *>(QStringLiteral("verticalCanvasPreview"));
	auto *responsiveControls =
		liveTab.findChild<QWidget *>(QStringLiteral("verticalCanvasSettingsSection"));
	auto *responsivePreviewColumn =
		liveTab.findChild<QWidget *>(QStringLiteral("verticalPreviewColumn"));
	...
	clipcoach::test::expect(
		responsivePreview != nullptr && responsiveControls != nullptr &&
			widthHint != nullptr &&
			widthHint->isHidden() &&
			compositionCard->isVisible() &&
			responsiveControls->isVisible() &&
			compactBar->isVisible() &&
			geometryInTab(previewCard).bottom() <=
				geometryInTab(compactBar).top() &&
			compositionCard->width() <= liveTab.width() &&
			responsiveControls->width() <= liveTab.width(),
		"narrow Vertical tab must show the 9:16 preview before its compact actions and controls");
	previewOnly->click();
	application.processEvents();
	clipcoach::test::expect(
		responsivePreview->isVisible() && previewOnly->isVisible() &&
			responsivePreview->width() > 315 &&
			compactBar->isHidden() && compositionCard->isHidden() &&
			responsiveControls->isHidden(),
		"preview-only mode must keep the camera visible while hiding canvas controls");
	previewOnly->click();
	application.processEvents();
	clipcoach::test::expect(
		compactBar->isVisible() && compositionCard->isVisible() &&
			responsiveControls->isVisible(),
	...
	clipcoach::test::expect(
		widthHint->isHidden() &&
			responsivePreview->width() >= 270 &&
			responsivePreview->height() >= 480 &&
			responsivePreviewColumn != nullptr &&
			compositionCard->parentWidget() == responsivePreviewColumn &&
			geometryInTab(responsiveControls).left() >=
				geometryInTab(responsivePreviewColumn).right(),
	...
	clipcoach::test::expect(
		canvasCard != nullptr && positionCard != nullptr &&
	...
	application.processEvents();
	clipcoach::test::expect(vertical.settings().zoomPercent == 160 &&
				       vertical.settings().panXPercent == -25 &&
				       vertical.settings().panYPercent == 35 &&
				       zoom->value() == 160,
	...
	auto *settingsDescription =
		dock.findChild<QWidget *>(QStringLiteral("verticalSettingsDescription"));
	...
	clipcoach::test::expect(
		dock.findChild<QComboBox *>(QStringLiteral("verticalTemplateCombo")) == nullptr &&
			dock.findChild<QWidget *>(QStringLiteral("verticalVisibleLayersSection")) == nullptr,
	...
	clipcoach::test::expect(
		scroll != nullptr && scroll->verticalScrollBarPolicy() == Qt::ScrollBarAsNeeded &&
	...
	resolution->setCurrentIndex(
		resolution->findData(static_cast<int>(clipcoach::VerticalResolution::Portrait720)));
	application.processEvents();
	auto *verticalStatus =
		dock.findChild<QWidget *>(QStringLiteral("verticalCanvasStatusCard"));
	auto *verticalStatusValue =
		verticalStatus != nullptr
			? verticalStatus->findChild<QLabel *>(
				  QStringLiteral("StatusValue"))
			: nullptr;
	clipcoach::test::expect(vertical.settings().outputMode == clipcoach::CaptureOutputMode::Vertical &&
					...
					verticalStatusValue->text() ==
						QStringLiteral("720×1280"),
	...
	clipcoach::ui::MainDock setupDock(
		[](const char *key) { return QString::fromUtf8(key); }, nullptr,
		&settings, nullptr, nullptr, {}, &vertical, nullptr, nullptr, nullptr,
		nullptr, nullptr, &setupVoice, {}, bridge);
	setupDock.showInitialSetupIfNeeded();
	auto *setupDialog = setupDock.findChild<QWizard *>(
		QStringLiteral("clipXtudioInitialSetup"));
	auto *setupScene = setupDialog != nullptr
		? setupDialog->findChild<QComboBox *>(
			QStringLiteral("initialSetupVerticalScene"))
		: nullptr;
	auto *setupMicrophone = setupDialog != nullptr
		? setupDialog->findChild<QComboBox *>(
			QStringLiteral("initialSetupMicrophone"))
		: nullptr;
	auto *setupVoiceEnabled = setupDialog != nullptr
		? setupDialog->findChild<QCheckBox *>(QStringLiteral("initialSetupVoiceEnabled"))
		: nullptr;
	auto *setupOutputMode = setupDialog != nullptr
		? setupDialog->findChild<QComboBox *>(QStringLiteral("initialSetupOutputMode"))
		: nullptr;
	clipcoach::test::expect(
		setupDialog != nullptr && setupScene != nullptr && setupMicrophone != nullptr &&
	...
	setupMicrophone->setCurrentIndex(
		setupMicrophone->findData(QStringLiteral("Shure Mic")));
	...
	setupDock.showInitialSetupIfNeeded();
	clipcoach::test::expect(
		setupDock.findChild<QDialog *>(QStringLiteral("clipXtudioInitialSetup")) == nullptr,
	...
	setupDock.showInitialSetupIfNeeded();
	auto *updatedSetup = setupDock.findChild<QWizard *>(
		QStringLiteral("clipXtudioInitialSetup"));
