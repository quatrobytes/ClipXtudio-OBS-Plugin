...
int main()
{
	...
	const auto directory =
		...
		("clipcoach-ffmpeg-" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
	std::filesystem::create_directories(directory);
	const auto source = directory / "source.mp4";

	QProcess fixture;
	fixture.start(QStringLiteral(FFMPEG_EXECUTABLE),
		      {QStringLiteral("-hide_banner"), QStringLiteral("-loglevel"), QStringLiteral("error"),
		       QStringLiteral("-f"), QStringLiteral("lavfi"), QStringLiteral("-i"),
		       QStringLiteral("testsrc2=size=640x360:rate=30:duration=4"), QStringLiteral("-f"),
		       QStringLiteral("lavfi"), QStringLiteral("-i"), QStringLiteral("sine=frequency=1000:duration=4"),
		       QStringLiteral("-c:v"), QStringLiteral("libx264"), QStringLiteral("-pix_fmt"),
		       QStringLiteral("yuv420p"), QStringLiteral("-c:a"), QStringLiteral("aac"),
		       QStringLiteral("-shortest"), QString::fromStdString(source.u8string())});
	expect(fixture.waitForFinished(30000) && fixture.exitCode() == 0, "FFmpeg media fixture must be generated");

	{
		ExportManager missingRuntime(
			std::make_unique<exporting::FfmpegExportBackend>(directory / "missing-ffmpeg.exe"));
		...
		expect(missingJob.state == ExportJobState::Error &&
			       missingJob.error.find("reinstall ClipXtudio") != std::string::npos,
		       "missing bundled runtime must return an actionable error");
	}

	ExportManager manager(
		std::make_unique<exporting::FfmpegExportBackend>(std::filesystem::path(FFMPEG_EXECUTABLE)));
	...
	expect(ids.size() == 2, "Both fixture must enqueue horizontal and vertical outputs");
	expect(manager.waitUntilIdle(std::chrono::seconds(30)), "vertical FFmpeg export must finish");
	const auto horizontal = manager.job(ids.front());
	const auto job = manager.job(ids.back());
	expect(horizontal.state == ExportJobState::Done && std::filesystem::is_regular_file(horizontal.outputPath) &&
		       job.state == ExportJobState::Done && std::filesystem::is_regular_file(job.outputPath),
	       "horizontal and vertical FFmpeg exports must produce MP4 files");

	QProcess probe;
	probe.start(QStringLiteral(FFPROBE_EXECUTABLE),
		    {QStringLiteral("-v"), QStringLiteral("error"), QStringLiteral("-select_streams"),
		     QStringLiteral("v:0"), QStringLiteral("-show_entries"), QStringLiteral("stream=width,height"),
	...
	durationProbe.start(QStringLiteral(FFPROBE_EXECUTABLE),
			    {QStringLiteral("-v"), QStringLiteral("error"), QStringLiteral("-show_entries"),
			     ...
			     QString::fromStdString(job.outputPath.u8string())});
	expect(durationProbe.waitForFinished(10000) && durationProbe.exitCode() == 0,
	       "ffprobe must read the exported duration");
	bool durationValid = false;
	const auto duration = durationProbe.readAllStandardOutput().trimmed().toDouble(&durationValid);
	...
	expect(trimmedJob.state == ExportJobState::Done && std::filesystem::is_regular_file(trimmedJob.outputPath),
	       "quick editor must create an MP4 for the selected range");
	QProcess trimProbe;
	trimProbe.start(QStringLiteral(FFPROBE_EXECUTABLE),
			{QStringLiteral("-v"), QStringLiteral("error"), QStringLiteral("-select_streams"),
			 QStringLiteral("v:0"), QStringLiteral("-show_entries"),
			 QStringLiteral("stream=width,height,avg_frame_rate:format=duration"), QStringLiteral("-of"),
			 QStringLiteral("json"), QString::fromStdString(trimmedJob.outputPath.u8string())});
	...
	expect(trimJson.contains("\"width\": 640") && trimJson.contains("\"height\": 360") &&
	...
	expect(cutJob.state == ExportJobState::Done && std::filesystem::is_regular_file(cutJob.outputPath),
	       "quick editor must concatenate all retained ranges into one MP4");
	QProcess cutProbe;
	cutProbe.start(QStringLiteral(FFPROBE_EXECUTABLE),
		       {QStringLiteral("-v"), QStringLiteral("error"), QStringLiteral("-show_entries"),
			QStringLiteral("format=duration"), QStringLiteral("-of"),
			QStringLiteral("default=noprint_wrappers=1:nokey=1"),
	...
	const auto cutDuration = cutProbe.readAllStandardOutput().trimmed().toDouble(&cutDurationValid);
