...
class MemorySecureStorage final
	: public clipcoach::security::SecureStorage {
public:
...
public:
	...
	beginAuthorization(
		clipcoach::integrations::ChatPlatform platform) override
	{
		return decltype(beginAuthorization(platform))::success(
			{"https://auth.example/connect", "csrf-state"});
	}
	...
	{
		refreshCalled = true;
		return decltype(refreshCredential(
			clipcoach::integrations::ChatPlatform::Twitch,
			""))::success(
...
int main()
{
	...
	expect(authorization.succeeded() &&
		       manager.state(ChatPlatform::Twitch) ==
			       ChatConnectionState::Connecting,
	       "Twitch must begin OAuth connection");
	std::string error;
	expect(!manager.completeConnect(ChatPlatform::Twitch, "code",
					"wrong-state", &error),
	...
	expect(poll.state == ChatConnectionState::Connected &&
		       poll.messages.size() == 1,
	       "connected Twitch integration must read messages");
	transport.networkFails = true;
	poll = manager.poll(ChatPlatform::Twitch);
	expect(poll.state == ChatConnectionState::NetworkError &&
		       !poll.error.empty(),
	...
	expect(pulseService.configure(pulseConfig),
	       "Chat Pulse service must configure for integration sync");
	ChatIntegrationController controller(manager, pulseService);
	const auto sync = controller.sync(ChatPlatform::YouTube);
	expect(sync.state == ChatConnectionState::Connected &&
		       sync.messageCount == 1 && sync.pulseCount == 1 &&
	...
	expect(!kick.succeeded() &&
		       manager.state(ChatPlatform::Kick) ==
	...
	expect(!freeManager.beginConnect(ChatPlatform::YouTube).succeeded() &&
		       freeManager.state(ChatPlatform::YouTube) ==
