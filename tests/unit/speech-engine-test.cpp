...
public:
	...
	clipcoach::SpeechRecognitionResult transcribe(const clipcoach::AudioSegment &, std::string_view language,
						      std::string_view token) override
	{
		++calls;
		receivedToken = std::string(token);
		return {"clip", std::string(language), 0.95, clipcoach::SpeechRecognitionError::None, {}};
	}
};
...
int main()
{
	...
	CloudSpeechEngine cloud(transport, [] { return "ephemeral-token"; }, false);
	auto result = cloud.recognize(validSegment(), "es");
	clipcoach::test::expect(result.error == SpeechRecognitionError::ConsentRequired && transport->calls == 0,
	...
	clipcoach::test::expect(result.succeeded() && transport->calls == 1 &&
	...
	clipcoach::test::expect(result.error == SpeechRecognitionError::SegmentTooLong && transport->calls == 1,
				"cloud engine must reject audio over its privacy limit");

	int localCalls = 0;
	LocalSpeechEngine local([&localCalls](const AudioSegment &, std::string_view language) {
		++localCalls;
		return SpeechRecognitionResult{"save that",
					       std::string(language),
					       0.9,
	...
	clipcoach::test::expect(result.succeeded() && localCalls == 1, "local transcriber must be injectable");
