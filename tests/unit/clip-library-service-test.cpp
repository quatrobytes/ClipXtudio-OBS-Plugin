...
int main()
{
	...
	clipcoach::test::expect(sessionPromise.get_future().get().success, "async session insert must succeed");
	...
	for (const auto &path : {clip.filePath, clip.thumbnailPath, clip.transcriptPath, clip.subtitlePath}) {
		std::ofstream asset(path, std::ios::binary);
		asset << "test";
	}
	std::promise<clipcoach::storage::StorageStatus> clipPromise;
	service.storeClip(clip, [&clipPromise](auto status) { clipPromise.set_value(std::move(status)); });
	clipcoach::test::expect(clipPromise.get_future().get().success, "async clip insert must succeed");
	std::promise<clipcoach::storage::StorageStatus> thumbnailPromise;
	service.updateThumbnail("clip-test-001", clip.thumbnailPath,
				[&thumbnailPromise](auto status) { thumbnailPromise.set_value(std::move(status)); });
	clipcoach::test::expect(thumbnailPromise.get_future().get().success, "async thumbnail update must succeed");
	...
	clipcoach::test::expect(query.result.value.front().thumbnailPath.filename() == "service-thumbnail.jpg",
	...
	std::promise<clipcoach::storage::RepositoryResult<clipcoach::storage::ClipLibrarySnapshot>> snapshotPromise;
	service.loadSnapshotAsync("session-test-001", 100,
				  [&snapshotPromise](auto result) { snapshotPromise.set_value(std::move(result)); });
	const auto snapshot = snapshotPromise.get_future().get();
	clipcoach::test::expect(snapshot.success && snapshot.value.clips.size() == 1 &&
	...
	service.storeExportJob(verticalJob,
			       [&exportPromise](auto status) { exportPromise.set_value(std::move(status)); });
	...
	std::promise<clipcoach::storage::RepositoryResult<std::vector<clipcoach::ClipMetadata>>> variantsPromise;
	service.listRecentAsync(100, [&variantsPromise](auto result) { variantsPromise.set_value(std::move(result)); });
	const auto variants = variantsPromise.get_future().get();
	const auto verticalVariant = std::find_if(variants.value.begin(), variants.value.end(), [](const auto &item) {
		return item.orientation == clipcoach::ClipOrientation::Vertical;
	});
	clipcoach::test::expect(
		variants.success && variants.value.size() == 2 && verticalVariant != variants.value.end() &&
			verticalVariant->filePath == verticalPath && verticalVariant->durationSeconds == 30,
	...
	service.deleteClips({"clip-test-001", verticalVariantId},
			    [&deletePromise](auto status) { deletePromise.set_value(std::move(status)); });
	clipcoach::test::expect(deletePromise.get_future().get().success, "async deletion must succeed");
	for (const auto &path :
	     {clip.filePath, clip.thumbnailPath, clip.transcriptPath, clip.subtitlePath, verticalPath}) {
		clipcoach::test::expect(!std::filesystem::exists(path), "deletion must remove every local clip asset");
	}
	std::promise<clipcoach::storage::RepositoryResult<std::vector<clipcoach::ClipMetadata>>> deletedQueryPromise;
	...
	clipcoach::test::expect(service.flush().success, "service must flush all queued storage work");
