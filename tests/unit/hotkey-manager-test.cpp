...
public:
	...
	void press(std::size_t index, bool pressed = true)
	{
		callbacks.at(index + 1)(pressed);
	}
...
int main()
{
	...
	services.defaultDurationSeconds = [] { return 45; };
	...
	manager.setErrorCallback(
		[&errorCalls](clipcoach::HotkeyAction, const std::string &) {
			++errorCalls;
		});
	...
	expect(manager.registerAll() && registrar.registerCalls == 9,
	...
	registrar.press(4);
	registrar.press(5);
	expect(captureDurations ==
		       std::vector<int>({45, 15, 30, 60, 120, 300}),
	...
	expect(captureDurations.size() == 6,
	       "key release must not execute an action");

	manager.unregisterAll();
	expect(registrar.unregistered.size() == 9 &&
		       registrar.callbacks.empty(),
	...
	inactiveServices.defaultDurationSeconds = [] { return 30; };
	inactiveServices.capture = [](int) {
		return HotkeyActionResult::fail(
			"Replay Buffer is not active");
	};
	clipcoach::HotkeyManager inactive(inactiveRegistrar,
					 std::move(inactiveServices));
	inactive.setErrorCallback(
		[&errorCalls](clipcoach::HotkeyAction, const std::string &) {
			++errorCalls;
		});
	...
	expect(errorCalls == 1,
	       "Replay Buffer failure must be reported without throwing");

	MockRegistrar failingRegistrar;
	failingRegistrar.failAtCall = 4;
	clipcoach::HotkeyManager failing(failingRegistrar, {});
	expect(!failing.registerAll(),
	       "partial native registration must fail atomically");
	expect(failingRegistrar.unregistered.size() == 3 &&
		       failingRegistrar.callbacks.empty(),
