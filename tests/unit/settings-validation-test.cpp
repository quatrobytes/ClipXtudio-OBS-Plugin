...
int main()
{
	...
	clipcoach::test::expect(manager.load(&error),
			       "validation fixture must load defaults");
	...
	clipcoach::test::expect(!manager.save(invalid, &error),
			       "relative media directories must be rejected");

	invalid = valid;
	invalid.fileNameTemplate = "{date}_{unknown}";
	clipcoach::test::expect(!manager.save(invalid, &error),
			       "unknown filename tokens must be rejected");

	invalid = valid;
	invalid.fileNameTemplate = "../{date}";
	clipcoach::test::expect(!manager.save(invalid, &error),
			       "filename templates must reject path traversal");
	...
	clipcoach::test::expect(
		!manager.save(invalid, &error),
		"duration plus pre/post-roll must fit Replay Buffer limits");

	invalid = valid;
	invalid.exportFps = 25;
	clipcoach::test::expect(!manager.save(invalid, &error),
			       "unsupported export FPS must fail");

	invalid = valid;
	invalid.verticalResolution =
		clipcoach::VerticalResolution::Portrait720;
	clipcoach::test::expect(!manager.save(invalid, &error),
			       "vertical preset and dimensions must stay consistent");

	invalid = valid;
	invalid.save15Hotkey = invalid.markMomentHotkey;
	clipcoach::test::expect(!manager.save(invalid, &error),
			       "duplicate non-empty hotkeys must fail");

	invalid = valid;
	invalid.voiceAudioSourceName = std::string(513, 'x');
	clipcoach::test::expect(!manager.save(invalid, &error),
			       "OBS audio source names must be bounded");

	clipcoach::test::expect(manager.settings().fileNameTemplate ==
