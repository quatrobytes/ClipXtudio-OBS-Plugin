...
struct SpeechRecognitionResult {
	...
	[[nodiscard]] bool succeeded() const noexcept { return error == SpeechRecognitionError::None; }
};

class SpeechEngine {
public:
	virtual ~SpeechEngine() = default;
	[[nodiscard]] virtual SpeechRecognitionResult recognize(const AudioSegment &segment,
								std::string_view languageHint) = 0;
};

class LocalSpeechEngine final : public SpeechEngine {
public:
	using Transcriber = std::function<SpeechRecognitionResult(const AudioSegment &, std::string_view)>;

	explicit LocalSpeechEngine(Transcriber transcriber,
				   std::chrono::milliseconds maximumSegment = std::chrono::seconds(10));

	[[nodiscard]] SpeechRecognitionResult recognize(const AudioSegment &segment,
...
public:
	...
	transcribe(const AudioSegment &segment, std::string_view languageHint, std::string_view shortLivedToken) = 0;
};
...
public:
	...
	CloudSpeechEngine(std::shared_ptr<CloudSpeechTransport> transport, TokenProvider tokenProvider,
			  bool explicitConsent, std::chrono::milliseconds maximumSegment = std::chrono::seconds(8));
