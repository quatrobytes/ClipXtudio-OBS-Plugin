...
public:
	...
	bestMatch(std::string_view transcript,
...
struct VoiceTriggerConfiguration {
	...
	std::vector<std::string> phrases{
		"clip", "guarda eso", "eso va pa TikTok", "modo grinch",
...
public:
	VoiceTriggerService(SpeechEngine &speechEngine,
			    TriggerEngine &triggerEngine, bool proUnlocked);

	[[nodiscard]] bool
	configure(const VoiceTriggerConfiguration &configuration,
		  std::string *error = nullptr);
	[[nodiscard]] const VoiceTriggerConfiguration &
	configuration() const noexcept;
	void setProUnlocked(bool unlocked) noexcept;

	[[nodiscard]] VoiceTriggerResult
	process(const AudioSegment &segment);
	[[nodiscard]] VoiceTriggerResult
	processRecognition(const SpeechRecognitionResult &recognition,
