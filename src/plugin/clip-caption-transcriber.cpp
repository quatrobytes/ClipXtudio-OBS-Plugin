...
void deliver(ClipCaptionTranscriber::Completion completion, ClipTranscriptionResult result)
{
	if (!completion)
		return;
...
void deliverProgress(const ClipCaptionTranscriber::Progress &progress, int percentage)
{
	if (!progress)
		return;
	...
	if (auto *application = QCoreApplication::instance()) {
		QMetaObject::invokeMethod(
			application, [progress, percentage] { progress(percentage); }, Qt::QueuedConnection);
		return;
	}
	progress(percentage);
}
...
public:
	Impl(std::filesystem::path ffmpegExecutable, std::filesystem::path whisperModel)
	...
	void enqueue(std::filesystem::path mediaPath, std::string language, Progress progress, Completion completion)
	{
		{
			std::lock_guard lock(mutex_);
			jobs_.push_back({std::move(mediaPath), std::move(language), std::move(progress),
					 std::move(completion)});
		}
...
private:
	...
	void run()
	{
		while (!stopping_.load()) {
			...
			deliver(std::move(job.completion), transcribe(job.mediaPath, job.language, job.progress));
		}
	}

	ClipTranscriptionResult transcribe(const std::filesystem::path &mediaPath, const std::string &language,
					   const Progress &progress)
	{
		...
		QProcess ffmpeg;
		ffmpeg.setProgram(QString::fromStdString(ffmpegExecutable_.u8string()));
		ffmpeg.setArguments(
			{QStringLiteral("-hide_banner"), QStringLiteral("-loglevel"), QStringLiteral("error"),
			 QStringLiteral("-i"), QString::fromStdString(mediaPath.u8string()), QStringLiteral("-vn"),
		...
		if (ffmpeg.exitStatus() != QProcess::NormalExit || ffmpeg.exitCode() != 0 || pcm.size() < 2) {
			const auto details = QString::fromUtf8(ffmpeg.readAllStandardError()).trimmed();
			return {false,
				{},
				details.isEmpty() ? "The clip has no readable audio" : details.toStdString()};
		}
		...
		for (qsizetype index = 0; index + 1 < pcm.size(); index += 2) {
			const auto value =
				static_cast<std::int16_t>(static_cast<std::uint16_t>(bytes[index]) |
		...
		parameters.n_threads =
			static_cast<int>(std::min(4u, std::max(1u, std::thread::hardware_concurrency())));
		...
		parameters.progress_callback = [](whisper_context *, whisper_state *, int value, void *data) {
			const auto *callback = static_cast<const Progress *>(data);
			if (callback != nullptr)
				deliverProgress(*callback, 20 + ((value * 78) / 100));
		};
		parameters.progress_callback_user_data = const_cast<Progress *>(&progress);
		...
		parameters.abort_callback_user_data =
			const_cast<std::chrono::steady_clock::time_point *>(&transcriptionDeadline);
		const std::string normalizedLanguage = language == "es" || language == "en" ? language : "auto";
		...
		for (int index = 0; index < segmentCount; ++index) {
			if (const char *segment = whisper_full_get_segment_text(context_, index))
				transcript += segment;
		}
		transcript = trim(std::move(transcript));
		if (transcript.empty())
			return {false, {}, "No spoken words were detected in this clip"};
	...
	bool ensureModelLoaded()
	{
		...
		if (!std::filesystem::is_regular_file(whisperModel_, error)) {
			modelError_ = "The bundled multilingual speech model is unavailable";
			return false;
		}
		...
		context_ = whisper_init_from_file_with_params(modelPath.c_str(), parameters);
		if (context_ == nullptr) {
			modelError_ = "The bundled multilingual speech model could not be loaded";
			return false;
		}
		return true;
	}
...
ClipCaptionTranscriber::ClipCaptionTranscriber(std::filesystem::path ffmpegExecutable,
					       std::filesystem::path whisperModel)
	: impl_(std::make_unique<Impl>(std::move(ffmpegExecutable), std::move(whisperModel)))
{
}

ClipCaptionTranscriber::~ClipCaptionTranscriber() = default;

void ClipCaptionTranscriber::transcribe(std::filesystem::path mediaPath, std::string language, Progress progress,
					Completion completion)
{
	impl_->enqueue(std::move(mediaPath), std::move(language), std::move(progress), std::move(completion));
}
