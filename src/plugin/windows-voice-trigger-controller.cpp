...
bool sameVoiceSettings(const Settings &left, const Settings &right)
{
	return left.triggerVoiceEnabled == right.triggerVoiceEnabled &&
	       ...
	       left.triggerSensitivity == right.triggerSensitivity &&
	       left.preRollSeconds == right.preRollSeconds &&
	       left.postRollSeconds == right.postRollSeconds &&
	       left.triggerAction == right.triggerAction &&
	       ...
	       left.voiceTriggerLanguage == right.voiceTriggerLanguage &&
	       left.voiceSpeechMode == right.voiceSpeechMode;
}
...
public:
	...
	[[nodiscard]] const std::string &lastTranscript() const noexcept
	{
		return lastTranscript_;
	}

	SpeechRecognitionResult recognize(const AudioSegment &segment,
					  std::string_view languageHint) override
	{
		if (context_ == nullptr)
			return {{}, {}, 0.0, SpeechRecognitionError::Unavailable,
				"The local speech model is not loaded"};
		if (segment.sampleRateHz != kWhisperSampleRate ||
		    segment.pcmMono.empty())
			return {{}, {}, 0.0, SpeechRecognitionError::InvalidAudio,
		...
		if (whisper_full(context_, params, samples.data(),
		...
		for (int segmentIndex = 0; segmentIndex < segments; ++segmentIndex) {
			if (const char *text =
				    whisper_full_get_segment_text(context_, segmentIndex))
				transcript += text;
			const int tokens =
				whisper_full_n_tokens(context_, segmentIndex);
			for (int token = 0; token < tokens; ++token) {
				const float probability = whisper_full_get_token_p(
					context_, segmentIndex, token);
		...
		const double confidence = probabilityCount > 0
						  ? probabilityTotal / probabilityCount
						  : 0.0;
		return {lastTranscript_, language.empty() ? "auto" : language,
			confidence, SpeechRecognitionError::None, {}};
	}
...
std::vector<std::string> enumerateAudioSources()
{
	...
	collector.names.erase(std::unique(collector.names.begin(),
					  collector.names.end()),
			      collector.names.end());
...
bool likelyMicrophone(obs_source_t *source)
{
	...
	const char *name = obs_source_get_name(source);
	std::string value = std::string(id ? id : "") + " " +
			    std::string(name ? name : "");
	std::transform(value.begin(), value.end(), value.begin(),
		       [](unsigned char character) {
			       return static_cast<char>(std::tolower(character));
		       });
	return value.find("input_capture") != std::string::npos ||
	       value.find("mic") != std::string::npos ||
...
obs_source_t *resolveAudioSource(const std::string &configuredName)
{
	...
	auto *selected = candidate.activeMicrophone != nullptr ? candidate.activeMicrophone
			 ...
			 : candidate.activeAudio != nullptr      ? candidate.activeAudio
							      : candidate.audio;
	for (auto *source : {candidate.activeMicrophone, candidate.microphone,
			    candidate.activeAudio, candidate.audio})
...
public:
	Impl(WindowsVoiceTriggerController &owner, TriggerEngine &engine)
		: owner_(owner), engine_(engine), service_(speech_, engine, false)
	{
	}
	...
	std::vector<std::string> availableSources() const
	{
		return enumerateAudioSources();
	}
...
private:
	void reconfigure()
	{
		...
		config.cooldown =
			std::chrono::seconds(settings_.triggerCooldownSeconds);
		...
		if (!proUnlocked_ ||
		    (!settings_.triggerVoiceEnabled &&
		     !settings_.triggerAudioSpikeEnabled &&
		     !settings_.triggerKeywordEnabled)) {
			publish(VoiceRuntimeState::Disabled,
				"OBS audio triggers are disabled");
			return;
		}
		...
		if (!obs_get_audio_info(&audioInfo)) {
			publish(VoiceRuntimeState::Error,
				"OBS audio configuration is unavailable");
			releaseSource();
			return;
		}
		const resample_info sourceInfo{audioInfo.samples_per_sec,
					       AUDIO_FORMAT_FLOAT_PLANAR,
					       audioInfo.speakers};
		const resample_info destinationInfo{kWhisperSampleRate,
						    AUDIO_FORMAT_16BIT,
						    SPEAKERS_MONO};
		resampler_ =
			audio_resampler_create(&destinationInfo, &sourceInfo);
		if (resampler_ == nullptr) {
			publish(VoiceRuntimeState::Error,
				"OBS audio could not be converted for local speech");
			releaseSource();
			return;
		}

		segmenter_.setSpeechThreshold(
			std::max(120, 1100 - settings_.triggerSensitivity * 10));
		...
		if (settings_.triggerVoiceEnabled ||
			...
			publish(VoiceRuntimeState::Listening,
				"Monitoring the selected OBS source for audio spikes");
		}
	...
	{
		if (source_ != nullptr)
			obs_source_remove_audio_capture_callback(source_, audioCaptured,
							 this);
	...
	static void audioCaptured(void *data, obs_source_t *,
	...
	void onAudio(const audio_data *audio, bool)
	{
		...
		if (!running_.load() || audio == nullptr ||
		    audio->frames == 0 || resampler_ == nullptr)
			return;
		...
		if (!audio_resampler_resample(resampler_, output, &outputFrames,
					      &timestampOffset, input,
					      audio->frames) ||
		...
		if (!audioReceiving_.exchange(true) && speech_.loaded())
			publish(VoiceRuntimeState::Listening,
				"Listening to the selected OBS audio source");
		const auto *samples =
			reinterpret_cast<const std::int16_t *>(output[0]);
		if (settings_.triggerAudioSpikeEnabled) {
			const auto now = std::chrono::steady_clock::now();
			if (now - lastAudioSpikeSignal_ >=
			    std::chrono::milliseconds(250)) {
				long double squareSum = 0.0;
				for (std::size_t index = 0; index < outputFrames;
				     ++index) {
					const auto sample =
						static_cast<long double>(samples[index]);
					squareSum += sample * sample;
				}
				const auto rms = std::sqrt(
					static_cast<double>(squareSum /
							    outputFrames));
				TriggerSignal signal;
				signal.type = SmartTriggerType::AudioSpike;
				signal.audioIntensity =
					std::clamp(rms / 6000.0, 0.0, 1.0);
				signal.durationSeconds = std::max(
					settings_constraints::kMinClipDurationSeconds,
					settings_.preRollSeconds +
						settings_.postRollSeconds);
		...
		for (auto &segment : segments) {
			while (queue_.size() >= kMaximumQueuedSegments)
				queue_.pop_front();
			blog(LOG_INFO,
			     "[ClipXtudio] Voice utterance queued duration_ms=%lld",
	...
	void run()
	{
		...
		publish(audioReceiving_.load() ? VoiceRuntimeState::Listening
					       : VoiceRuntimeState::WaitingForAudio,
			audioReceiving_.load()
				? "Listening to the selected OBS audio source"
		...
		while (running_.load()) {
			...
			if (settings_.triggerVoiceEnabled)
				...
				const auto recognition = speech_.recognize(
					segment, settings_.voiceTriggerLanguage);
				result.status =
					recognition.succeeded()
						? VoiceTriggerStatus::NoPhraseMatch
						: VoiceTriggerStatus::RecognitionFailed;
				result.message = recognition.errorMessage;
			}
			const auto transcript = speech_.lastTranscript();
			if (settings_.triggerKeywordEnabled &&
			    !transcript.empty()) {
				...
				keywordSignal.durationSeconds = std::max(
					settings_constraints::kMinClipDurationSeconds,
					settings_.preRollSeconds +
						settings_.postRollSeconds);
			...
			if (result.status == VoiceTriggerStatus::EventCreated)
				blog(LOG_INFO,
				     "[ClipXtudio] Voice command matched; trigger event created");
			else if (result.status == VoiceTriggerStatus::LowConfidence)
				blog(LOG_DEBUG,
				     "[ClipXtudio] Speech ignored because confidence was below threshold");
			else if (result.status == VoiceTriggerStatus::NoPhraseMatch)
				blog(LOG_DEBUG,
				     "[ClipXtudio] Speech recognized without a configured command match");
			if (!transcript.empty())
				publish(VoiceRuntimeState::Listening,
	...
	void publish(VoiceRuntimeState state, std::string message,
		     std::string transcript = {})
	{
		...
		blog(LOG_INFO,
		     "[ClipXtudio] Voice runtime state=%d model=%s audio=%s source=%s",
		     static_cast<int>(snapshot.state),
		     snapshot.modelLoaded ? "ready" : "loading",
		     snapshot.audioReceiving ? "receiving" : "waiting",
		     snapshot.sourceName.empty() ? "(none)"
						 : snapshot.sourceName.c_str());
		if (callback)
...
WindowsVoiceTriggerController::WindowsVoiceTriggerController(
	TriggerEngine &engine, QObject *parent)
	: QObject(parent), impl_(std::make_unique<Impl>(*this, engine))
{
}
...
WindowsVoiceTriggerController::availableAudioSources() const
{
