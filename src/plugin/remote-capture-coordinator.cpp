...
std::string captureErrorCode(CaptureError error)
{
	...
	case CaptureError::ReplayBufferInactive:
		return "REPLAY_BUFFER_INACTIVE";
	case CaptureError::ReplayBufferWarmingUp:
		return "REPLAY_BUFFER_WARMING_UP";
...
RemoteCaptureCoordinator::RemoteCaptureCoordinator(ClipManager &clips, ExportManager &exports,
	...
	: clips_(clips),
	  exports_(exports),
	  settings_(settings),
	  ensureCapacity_(std::move(ensureCapacity)),
...
void RemoteCaptureCoordinator::capture(const remote::RemoteCapturePlan &plan, Completion completion)
{
	...
	if (ensureCapacity_)
		ensureCapacity_(plan.replayWindowSeconds);
	const auto result = clips_.captureTriggered(plan.replayWindowSeconds, TriggerType::Manual, "remote_clipper", 0,
						    plan.requestedBy);
	if (!result.accepted) {
...
ExportRequest RemoteCaptureCoordinator::requestFor(const ClipMetadata &clip) const
{
	...
	request.outputDirectory = settings.exportDirectory.empty() ? clip.filePath.parent_path() / "ClipXtudio Remote"
