...
RemoteCommandPoller::~RemoteCommandPoller()
{
	stop();
}
...
{
	if (running_ && !inFlight_ && !authorizationPaused_)
		schedule(0);
}
...
void RemoteCommandPoller::setCommandsCallback(CommandsCallback callback)
{
	commandsCallback_ = std::move(callback);
}
void RemoteCommandPoller::setStatusCallback(StatusCallback callback)
{
	statusCallback_ = std::move(callback);
}
remote::RemoteClipperStatus RemoteCommandPoller::status() const
{
	return status_;
}
...
void RemoteCommandPoller::runCycle()
{
	...
	api_.heartbeat(
		heartbeat, token, [self, token, deviceId = heartbeat.deviceActivationId, generation](auto result) {
			if (self.isNull())
				return;
			if (generation != self->credentialGeneration_)
				return;
			if (!self->running_) {
			...
			self->api_.commands(deviceId, token, [self, generation](auto commandsResult) {
				...
				if (!commandsResult.succeeded()) {
					self->fail(commandsResult.error);
					return;
				}
				self->consecutiveFailures_ = 0;
...
{
	if (!running_ || authorizationPaused_ || !timer_)
		return;
