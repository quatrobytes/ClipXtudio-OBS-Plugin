...
{
	static const QRegularExpression pattern(QStringLiteral(R"(^v?(\d+)\.(\d+)\.(\d+)(?:-([0-9A-Za-z.-]+))?$)"));
...
QtUpdateChecker::QtUpdateChecker(QList<QUrl> manifestUrls, QNetworkAccessManager *manager) : manager_(manager)
{
...
void QtUpdateChecker::check(const QString &currentVersion, Completion completion)
{
	if (!configured_) {
		if (completion)
			completion({false, false, {}, {}, {}, {}, 0, QStringLiteral("UPDATE_URL_NOT_CONFIGURED")});
		return;
	}
	if (!QSslSocket::supportsSsl() &&
	    std::any_of(manifestUrls_.cbegin(), manifestUrls_.cend(), [](const QUrl &url) {
		    return url.scheme().compare(QStringLiteral("https"), Qt::CaseInsensitive) == 0;
	    })) {
		if (completion)
			completion({false, false, {}, {}, {}, {}, 0, QStringLiteral("UPDATE_TLS_BACKEND_UNAVAILABLE")});
		return;
	}
	checkCandidate(currentVersion, 0, std::move(completion));
}

void QtUpdateChecker::checkCandidate(const QString &currentVersion, qsizetype candidateIndex, Completion completion)
{
	...
	QObject::connect(reply, &QNetworkReply::finished, reply,
			 [this, reply, currentVersion, candidateIndex, completion = std::move(completion)]() mutable {
				 auto result = parseReply(*reply, currentVersion);
				 reply->deleteLater();
				 if (!result.success && candidateIndex + 1 < manifestUrls_.size()) {
					 checkCandidate(currentVersion, candidateIndex + 1, std::move(completion));
					 return;
				 }
				 if (completion)
...
UpdateCheckResult QtUpdateChecker::parseReply(QNetworkReply &reply, const QString &currentVersion) const
{
	...
	if (!validTransportUrl(reply.url()))
		return {false, false, {}, {}, {}, {}, 0, QStringLiteral("UPDATE_REDIRECT_REJECTED")};
	...
	if (object.contains(QStringLiteral("available")) && !object.value(QStringLiteral("available")).toBool(true))
		return {true, false, {}, {}, {}, {}, 0, {}};
	...
	if (!parseVersion(latestVersion) || !validTransportUrl(downloadUrl) ||
	    ...
	    !checksumPattern.match(sha256).hasMatch() || sizeBytes <= 0 || sizeBytes > 512LL * 1024LL * 1024LL)
		return {false, false, {}, {}, {}, {}, 0, QStringLiteral("UPDATE_RESPONSE_INVALID")};
	return {true,
