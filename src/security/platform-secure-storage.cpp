...
public:
	bool store(std::string_view, const SecretBytes &, std::string *error) override
	{
		if (error)
	...
	{
		return {{}, SecureStorageError::Unavailable, "Platform secure storage is unavailable"};
	}
...
public:
	explicit WindowsCredentialSecureStorage(std::string serviceName) : serviceName_(std::move(serviceName)) {}

	bool store(std::string_view key, const SecretBytes &secret, std::string *error) override
	{
		if (secret.empty() || secret.size() > CRED_MAX_CREDENTIAL_BLOB_SIZE) {
			if (error)
				*error = "Secret exceeds Windows Credential Manager limits";
			return false;
		}
		...
		if (!CredWriteW(&credential, 0)) {
			if (error)
				*error = "Windows Credential Manager write failed";
			return false;
		}
		return true;
	}
	...
	SecretReadResult load(std::string_view key) override
	{
		const auto target = targetName(key);
		if (!target)
			return {{}, SecureStorageError::InvalidData, "Secure storage key is invalid UTF-8"};
		PCREDENTIALW credential = nullptr;
		if (!CredReadW(target->c_str(), CRED_TYPE_GENERIC, 0, &credential)) {
			return {{},
				GetLastError() == ERROR_NOT_FOUND ? SecureStorageError::NotFound
								  : SecureStorageError::PlatformFailure,
				"Windows Credential Manager read failed"};
		}
		SecretBytes result(credential->CredentialBlob,
...
private:
	std::optional<std::wstring> targetName(std::string_view key) const
	{
...
std::unique_ptr<SecureStorage> createPlatformSecureStorage(std::string serviceName)
{
#ifdef _WIN32
	return std::make_unique<WindowsCredentialSecureStorage>(std::move(serviceName));
#else
	(void)serviceName;
