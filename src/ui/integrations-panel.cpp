...
IntegrationsPanel::IntegrationsPanel(TranslationFunction translator, integrations::ChatIntegrationManager *manager,
	  ...
	  opener_(std::move(opener))
{
	...
	if (manager_ == nullptr) {
		auto *notice = new QLabel(text(strings::kIntegrationUnavailable), this);
		notice->setObjectName(QStringLiteral("chatIntegrationUnavailableNotice"));
		notice->setProperty("notificationTone", QStringLiteral("warning"));
		notice->setWordWrap(true);
		layout->addWidget(notice);
	}
	addPlatform(integrations::ChatPlatform::Twitch, QStringLiteral("Twitch"),
		    QStringLiteral("twitchConnectionStatus"));
	addPlatform(integrations::ChatPlatform::YouTube, QStringLiteral("YouTube Live"),
		    QStringLiteral("youtubeConnectionStatus"));
	addPlatform(integrations::ChatPlatform::Kick, QStringLiteral("Kick"), QStringLiteral("kickConnectionStatus"));
	refresh();
}
...
void IntegrationsPanel::addPlatform(integrations::ChatPlatform platform, const QString &name,
				    const QString &statusObjectName)
{
	...
	auto *connectButton = new QPushButton(text(strings::kIntegrationConnect), row);
	connectButton->setObjectName(
		QStringLiteral("%1ConnectButton").arg(QString::fromUtf8(integrations::chatPlatformName(platform))));
	auto *disconnectButton = new QPushButton(text(strings::kIntegrationDisconnect), row);
	disconnectButton->setObjectName(
		QStringLiteral("%1DisconnectButton").arg(QString::fromUtf8(integrations::chatPlatformName(platform))));
	...
	connect(connectButton, &QPushButton::clicked, this, [this, platform] { beginConnect(platform); });
	connect(disconnectButton, &QPushButton::clicked, this, [this, platform] { disconnect(platform); });
...
void IntegrationsPanel::refresh()
{
	...
	for (auto &[platform, row] : rows_) {
		auto state = platform == integrations::ChatPlatform::Kick ? ChatConnectionState::ComingSoon
									  : ChatConnectionState::ProRequired;
		if (manager_ != nullptr)
			state = manager_->state(platform);
		row.status->setText(text(stateKey(state)));
		const auto canConnect = state == ChatConnectionState::Disconnected ||
		...
		row.disconnect->setEnabled(manager_ != nullptr && (state == ChatConnectionState::Connected ||
								   state == ChatConnectionState::NetworkError ||
								   state == ChatConnectionState::TokenExpired));
		row.connect->setVisible(platform != integrations::ChatPlatform::Kick);
		row.disconnect->setVisible(platform != integrations::ChatPlatform::Kick);
	}
}

void IntegrationsPanel::beginConnect(integrations::ChatPlatform platform)
{
	if (manager_ == nullptr)
		return;
	const auto request = manager_->beginConnect(platform);
	if (request.succeeded()) {
		const QUrl url(QString::fromStdString(request.value->authorizationUrl), QUrl::StrictMode);
...
void IntegrationsPanel::disconnect(integrations::ChatPlatform platform)
{
	if (manager_ == nullptr)
		return;
	std::string ignored;
