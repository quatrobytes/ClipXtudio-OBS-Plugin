...
{
	if (value == "mark_moment") return RemoteCommandType::MarkMoment;
	if (value == "save_clip_30") return RemoteCommandType::SaveClip30;
	if (value == "save_clip_60") return RemoteCommandType::SaveClip60;
	if (value == "save_custom") return RemoteCommandType::SaveCustom;
	if (value == "save_vertical") return RemoteCommandType::SaveVertical;
	if (value == "save_both") return RemoteCommandType::SaveBoth;
...
{
	switch (type) {
	case RemoteCommandType::MarkMoment: return "mark_moment";
	case RemoteCommandType::SaveClip30: return "save_clip_30";
	case RemoteCommandType::SaveClip60: return "save_clip_60";
	case RemoteCommandType::SaveCustom: return "save_custom";
	case RemoteCommandType::SaveVertical: return "save_vertical";
	case RemoteCommandType::SaveBoth: return "save_both";
	case RemoteCommandType::Unknown: return "unknown";
	}
	return "unknown";
}
...
RemoteCommandExecutor::RemoteCommandExecutor(CaptureAction captureAction, MarkMomentAction markMomentAction,
	...
	: captureAction_(std::move(captureAction)), markMomentAction_(std::move(markMomentAction)),
...
void RemoteCommandExecutor::submit(RemoteCommand command, Completion completion)
{
	if (!isValidCommandUuid(command.uuid)) {
		completion({command.uuid, false, {}, {}, 0, {}, {}, "INVALID_COMMAND_UUID",
			    "Remote command UUID is invalid"});
		return;
	}
	if (processed_.count(command.uuid) != 0 ||
	    std::any_of(queue_.begin(), queue_.end(), [&command](const Pending &pending) {
		    return pending.command.uuid == command.uuid;
	    })) {
		completion({command.uuid, false, {}, {}, 0, {}, {}, "DUPLICATE_COMMAND",
...
bool RemoteCommandExecutor::busy() const noexcept { return busy_; }
std::size_t RemoteCommandExecutor::queuedCount() const noexcept { return queue_.size(); }

std::optional<RemoteCapturePlan> RemoteCommandExecutor::planFor(const RemoteCommand &command,
								 ...
								 std::string *errorMessage)
{
	...
	if (command.type == RemoteCommandType::MarkMoment)
		return RemoteCapturePlan{command.uuid, command.type, 0, command.delayCompensationSeconds, 0,
					 false, false, command.note, command.requestedBy};

	int duration = command.durationSeconds;
	if (command.type == RemoteCommandType::SaveClip30) duration = 30;
	if (command.type == RemoteCommandType::SaveClip60) duration = 60;
	...
	return RemoteCapturePlan{command.uuid, command.type, duration, command.delayCompensationSeconds,
				 duration + command.delayCompensationSeconds,
				 command.type == RemoteCommandType::SaveVertical,
				 command.type == RemoteCommandType::SaveBoth, command.note,
				 command.requestedBy};
}
...
void RemoteCommandExecutor::executeNext()
{
	...
	if (pending.command.type == RemoteCommandType::MarkMoment) {
		if (!markMomentAction_) {
			done({pending.command.uuid, false, {}, {}, 0, {}, {}, "MARK_MOMENT_UNAVAILABLE",
	...
	if (!captureAction_) {
		done({pending.command.uuid, false, {}, {}, 0, {}, {}, "CAPTURE_UNAVAILABLE",
