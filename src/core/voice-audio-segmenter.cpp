...
VoiceAudioSegmenter::VoiceAudioSegmenter(VoiceAudioSegmenterConfig config)
	: config_(config)
{
...
std::vector<AudioSegment> VoiceAudioSegmenter::push(const std::int16_t *samples,
						     std::size_t count)
{
	...
	const auto remainingCapacity =
		maximumSegmentSamples > activeSamples_.size()
			? maximumSegmentSamples - activeSamples_.size()
			: 0;
	const auto acceptedCount = std::min(count, remainingCapacity);
	activeSamples_.insert(activeSamples_.end(), samples,
	...
	const bool maximumReached =
		activeSamples_.size() >= maximumSegmentSamples;
	const bool utteranceEnded =
		speechSamples_ >= minimumSpeechSamples &&
...
AudioSegment VoiceAudioSegmenter::finish()
{
	...
	segment.duration = std::chrono::milliseconds(
		segment.pcmMono.size() * 1000 /
		static_cast<std::size_t>(config_.sampleRateHz));
