...
std::string joinWords(const std::vector<std::string> &values,
...
std::size_t levenshteinDistance(std::string_view left,
				std::string_view right)
{
	...
	for (std::size_t leftIndex = 1; leftIndex <= left.size(); ++leftIndex) {
		current[0] = leftIndex;
		for (std::size_t rightIndex = 1; rightIndex <= right.size();
		     ++rightIndex) {
...
double similarity(std::string_view left, std::string_view right)
{
	...
	if (longest == 0)
		return 1.0;
	return 1.0 -
	       static_cast<double>(levenshteinDistance(left, right)) /
		       static_cast<double>(longest);
}

double matchingThreshold(int sensitivity)
{
	return 0.82 + static_cast<double>(std::clamp(sensitivity, 0, 100)) *
			      0.0015;
}
...
double confidenceThreshold(int sensitivity)
{
	...
	return 0.30 + static_cast<double>(std::clamp(sensitivity, 0, 100)) *
			      0.0025;
}

bool supportedLanguage(const std::string &language)
{
	return language == "auto" || language == "es" || language == "en" ||
	       language.rfind("es-", 0) == 0 || language.rfind("en-", 0) == 0;
}
...
std::optional<VoicePhraseMatch> VoicePhraseMatcher::bestMatch(
	std::string_view transcript, const std::vector<std::string> &phrases,
	int sensitivity)
{
	...
	for (const auto &configured : phrases) {
		...
		if (phraseWords.empty() ||
		    phraseWords.size() > transcriptWords.size())
			continue;
		for (std::size_t begin = 0;
		     begin + phraseWords.size() <= transcriptWords.size();
		     ++begin) {
			const auto candidate =
				joinWords(transcriptWords, begin,
					  phraseWords.size());
...
VoiceTriggerService::VoiceTriggerService(SpeechEngine &speechEngine,
...
bool VoiceTriggerService::configure(
	const VoiceTriggerConfiguration &configuration, std::string *error)
{
	if (error)
		error->clear();
	...
	if (configuration.sensitivity < 0 ||
	    ...
	    configuration.phrases.empty() ||
	    configuration.phrases.size() > 50 ||
	    !supportedLanguage(configuration.language) ||
	    std::any_of(configuration.phrases.begin(),
			configuration.phrases.end(),
			[](const std::string &phrase) {
				return VoicePhraseMatcher::normalize(phrase)
					       .empty() ||
				       phrase.size() > 120;
			})) {
		if (error)
	...
	for (const auto &phrase : configuration.phrases) {
		if (!normalizedPhrases
			     .insert(VoicePhraseMatcher::normalize(phrase))
			     .second) {
			if (error)
				*error =
					"Voice Trigger phrases must be unique";
			return false;
		}
	}
	...
	if (!triggerEngine_.setConfiguration(SmartTriggerType::Voice,
					     triggerConfig, error))
		return false;
...
const VoiceTriggerConfiguration &
VoiceTriggerService::configuration() const noexcept
{
...
{
	if (!proUnlocked_)
		return {VoiceTriggerStatus::ProRequired, {}, {},
			"Voice Trigger requires ClipXtudio Pro"};
	if (!configuration_.enabled)
		return {VoiceTriggerStatus::Disabled, {}, {},
			"Voice Trigger is disabled"};

	const auto recognition =
		speechEngine_.recognize(segment, configuration_.language);
	return processRecognition(recognition, segment.startedAt, segment.duration);
}

VoiceTriggerResult VoiceTriggerService::processRecognition(
	...
	std::chrono::milliseconds /*duration*/)
{
	...
	if (!recognition.succeeded())
		return {VoiceTriggerStatus::RecognitionFailed, {}, {},
			recognition.errorMessage};
	if (recognition.confidence <
	    confidenceThreshold(configuration_.sensitivity))
		return {VoiceTriggerStatus::LowConfidence, {}, {},
			"Speech confidence is below the configured threshold"};

	const auto match = VoicePhraseMatcher::bestMatch(
		recognition.transcript, configuration_.phrases,
		configuration_.sensitivity);
	if (!match)
		return {VoiceTriggerStatus::NoPhraseMatch, {}, {},
			"No configured voice phrase matched"};

	if (lastMatchAt_ &&
	    (occurredAt <= *lastMatchAt_ ||
	     occurredAt - *lastMatchAt_ <
		     configuration_.cooldown))
		return {VoiceTriggerStatus::Cooldown, {}, match,
			"Voice Trigger is in cooldown"};
	...
	signal.voiceConfidence =
		match->similarity == 1.0
			? 1.0
			: std::clamp(match->similarity * 0.8 +
					     recognition.confidence * 0.2,
				     0.0, 1.0);
	...
	signal.durationSeconds = std::max(
		5, configuration_.preRollSeconds + configuration_.postRollSeconds);
	...
	if (!result.event)
		return {result.rejection == TriggerRejection::Cooldown ||
			...
			{}, match, "Trigger engine rejected the voice event"};
